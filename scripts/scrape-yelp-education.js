#!/usr/bin/env node
/**
 * Yelp Education Consultants / College Counselors Scraper (Fusion API)
 *
 * Uses the Yelp Fusion API v3 to search for education consultants,
 * college counselors, SAT prep, study abroad consultants, and
 ...
 *   - ~5 calls per city per search term to get all results
 *
 * Output: output/education-consultants-yelp.csv
 *
 * Usage:
 *   node scripts/scrape-yelp-education.js --test                   # 5 cities
 *   node scripts/scrape-yelp-education.js --all-cities             # All cities
 *   node scripts/scrape-yelp-education.js --country US             # US only
 *   node scripts/scrape-yelp-education.js --country UK             # UK only
 *   node scripts/scrape-yelp-education.js --country CA             # Canada only
 *   node scripts/scrape-yelp-education.js --country ALL            # All countries
 *   node scripts/scrape-yelp-education.js --country ALL --resume   # Resume interrupted
 */
...
const DELAY_BETWEEN_TERMS = 1500;  // 1.5s between search terms within a city

const OUT_DIR = path.join(__dirname, '..', 'output');
const OUT_FILE = path.join(OUT_DIR, 'education-consultants-yelp.csv');
const PROGRESS_FILE = path.join(OUT_DIR, 'yelp-education-progress.json');
...
const SEARCH_TERMS = [
  { term: 'education consultant', categories: 'educationalservices,collegeuniv,testprep,tutoring' },
  { term: 'college counselor', categories: 'educationalservices,collegeuniv,counseling' },
  { term: 'college admissions consultant', categories: 'educationalservices,collegeuniv' },
  { term: 'SAT prep', categories: 'testprep,tutoring,educationalservices' },
  { term: 'study abroad consultant', categories: 'educationalservices,studyabroad,collegeuniv' },
];
...
/**
 * Parse a person's name from an education business name.
 * Handles patterns like "John Smith Tutoring", "Smith Education Consulting",
 * "Dr. Jane Doe College Prep", etc.
 */
function parseName(bizName) {
  if (!bizName) return { first_name: '', last_name: '' };

  let cleaned = bizName;

  // Strip common education business suffixes/words
  cleaned = cleaned
    .replace(/,?\s*(LLC|PLLC|PC|PLC|PA|P\.A\.|LLP|Inc\.?|Corp\.?)/gi, '')
    .replace(/\s*(Education(?:al)?\s+(?:Consulting|Consultants?|Services?|Group|Center|Centre)|College\s+(?:Consulting|Counseling|Counselor|Prep|Admissions)|Admissions\s+(?:Consulting|Consultants?|Counseling|Advisors?|Expert)|Test\s+Prep(?:aration)?|SAT\s+(?:Prep|Tutoring)|ACT\s+Prep|Tutoring\s+(?:Services?|Center|Centre|Group)|Study\s+Abroad|Academic\s+(?:Coaching|Consulting|Advisors?|Services?)|College\s+(?:Coach(?:ing)?|Planning|Solutions|Advisors?)|Consulting\s+(?:Group|Services?|Firm)|& Associates?|Associates?)/gi, '')
    .replace(/\s*(Tutoring|Tutor|Consulting|Consultant|Counseling|Counselor|Coaching|Coach|Prep|Academy|Institute|Learning|Center|Centre|Education|Educational|Services?|Solutions?|Advisors?|Advisory|Group)/gi, '')
    .trim();

  // Strip honorifics
  cleaned = cleaned.replace(/^(Dr\.?|Prof\.?|Mr\.?|Mrs\.?|Ms\.?)\s+/i, '').trim();
  ...
  // Skip if contains & or + (multiple people/partners)
  if (/\s[&+]\s|,\s+and\s+/i.test(cleaned)) return { first_name: '', last_name: '' };

  // Reject if the remaining string is a common business word
  const bizWords = /^(education|educational|academic|college|prep|test|sat|act|tutoring|tutor|consulting|consultant|counseling|counselor|learning|center|centre|academy|institute|services?|solutions?|group|advisors?|coaching|coach|global|premier|elite|top|best|bright|smart|genius|ivy|league|plus|pro)$/i;

  // Check if it looks like a person name: "First Last" or "First M. Last"
  if (cleaned && /^[A-Z][a-z]+(\s+[A-Z]\.?)?\s+[A-Z][a-z]+(-[A-Z][a-z]+)?$/i.test(cleaned)) {
    const parts = cleaned.split(/\s+/);
    const lastName = parts[parts.length - 1];
    if (bizWords.test(lastName)) return { first_name: '', last_name: '' };
    if (bizWords.test(parts[0])) return { first_name: '', last_name: '' };
...
/**
 * Search for education businesses in a specific location with a specific term.
 * Paginates through all results (max 240 per Yelp limit).
 */
async function searchCityTerm(cityInfo, searchTerm, categories, apiCallCount) {
  const results = [];
  let offset = 0;
  let totalResults = 0;
  let pages = 0;
...
/**
 * Search a city across all search terms, combining results.
 */
async function searchCity(cityInfo, apiCallCount) {
  ...
  for (let i = 0; i < SEARCH_TERMS.length; i++) {
    const { term, categories } = SEARCH_TERMS[i];
    const { results, totalResults, pages } = await searchCityTerm(cityInfo, term, categories, apiCallCount);
    totalApiCalls += pages;

    // Dedup within city by Yelp business ID
    let newFromTerm = 0;
    ...
    if (results.length > 0) {
      log(`    "${term}": ${results.length} results (${newFromTerm} new, ${totalResults} total available)`);
    }

    // Delay between search terms within same city
    if (i < SEARCH_TERMS.length - 1) {
      await sleep(DELAY_BETWEEN_TERMS);
    }
  }

  return { results: allResults, totalApiCalls };
}
...
async function main() {
  ...
  if (!isTest && !isAllCities && countryIdx === -1 && !isResume) {
    console.log('Yelp Education Consultants / College Counselors Scraper (Fusion API)');
    console.log('');
    ...
    console.log('');
    console.log('Usage:');
    console.log('  node scripts/scrape-yelp-education.js --test                   # Test mode (5 cities)');
    console.log('  node scripts/scrape-yelp-education.js --all-cities             # All cities, all countries');
    console.log('  node scripts/scrape-yelp-education.js --country US             # US cities only');
    console.log('  node scripts/scrape-yelp-education.js --country UK             # UK cities only');
    console.log('  node scripts/scrape-yelp-education.js --country CA             # Canada cities only');
    console.log('  node scripts/scrape-yelp-education.js --country ALL            # All countries');
    console.log('  node scripts/scrape-yelp-education.js --country ALL --resume   # Resume interrupted');
    process.exit(0);
  }
  ...
  if (!API_KEY) {
    ...
    console.error('  4. Run: YELP_API_KEY=your_key node scripts/scrape-yelp-education.js --test');
    process.exit(1);
  }
  ...
  log(`Yelp Education Consultants Scraper (Fusion API)`);
  log(`Country filter: ${countryFilter} | Total cities: ${cities.length} | Remaining: ${citiesToScrape.length} | Test: ${isTest}`);
  log(`Search terms: ${SEARCH_TERMS.length} (${SEARCH_TERMS.map(t => `"${t.term}"`).join(', ')})`);
  log(`Output: ${OUT_FILE}`);
  log('');

  // Validate API key with a test call
  log('Validating API key...');
  const testResult = await apiGet(`${API_BASE}/businesses/search?term=education+consultant&location=New+York&limit=1`);
  ...
  for (const cityInfo of citiesToScrape) {
    ...
    try {
      const { results, totalApiCalls } = await searchCity(cityInfo, apiCallCount);

      if (results.length === 0) {
        log(`  No results found across all search terms`);
        emptyPages++;
      ...
      log(`  Combined: ${results.length} unique businesses from ${SEARCH_TERMS.length} search terms`);
  ...
  log(`Search terms used: ${SEARCH_TERMS.length}`);
