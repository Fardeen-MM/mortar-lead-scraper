#!/usr/bin/env node
/**
 * Scrape DanceStudioDirectory.com
 *
 * Same OptimizeCDN template as MartialArtsSchoolsDirectory.com. Listing URLs
 * look like:
 *   /{state-or-country-slug}/{city-slug}/dance-studio/{business-slug}
 *
 * Strategy mirrors the martial-arts scraper: walk a fixed list of index pages
 * (50 US states + common countries), follow `?page=N` pagination, collect
 * every distinct listing URL, then fetch each listing page and pull the
 * JSON-LD LocalBusiness block.
 *
 * NOTE: as of this build the live dancestudiodirectory.com has essentially
 * no public listings (only a single test entry in Washington). The scraper
 * is implemented to the same contract as the martial-arts scraper so it
 * remains usable the instant new listings appear. It runs without error and
 * writes rows for whatever is reachable.
 */
...
const BASE = 'https://www.dancestudiodirectory.com';
const NICHE = 'dance-studio';
const SOURCE = 'dancestudiodirectory.com';

const OUTPUT_CSV = path.join(__dirname, '..', 'output', 'dance-studio-directory.csv');
const STATE_DIR = path.join(__dirname, '..', 'state');
const STATE_FILE = path.join(STATE_DIR, 'dance-studio-directory.json');
    ...
    const req = https.request(
      ...
      (res) => {
        ...
        res.on('end', () => {
          ...
          // Some index pages are returned via a client-side redirect script
          // like `window.location.href = "/california?google=..."`. Follow it.
          const jsRedirect = body.match(
...
function extractListingUrls(html, slugPath) {
  ...
  for (const m of matches) {
    ...
    // expect the shape /a/b/dance-studio/c -> 5 segments once split on '/'
...
async function collectListingUrls() {
  ...
  for (const slug of INDEX_SLUGS) {
    for (let page = 1; page <= MAX_PAGES_PER_INDEX; page++) {
      ...
      const urls = extractListingUrls(body, 'dance-studio');
  ...
  for (const s of scripts) {
    ...
    for (const node of nodes) {
      ...
      const isBusiness = types.some((t) =>
        /LocalBusiness|Organization|SportsActivityLocation|HealthClub|SportsClub|DanceStudio|EducationalOrganization|ExerciseGym|DanceGroup|PerformingArtsTheater/i.test(
          String(t || '')
        )
      );
...
async function main() {
  ...
  if (testMode) {
    ...
    INDEX_SLUGS.push('california', 'new-york', 'texas', 'florida', 'washington');
  } else if (limitStates) {
