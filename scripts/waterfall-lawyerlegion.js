#!/usr/bin/env node
/**
 * Email-waterfall the 2,305 LawyerLegion rows that have name + domain.
 */
const fs = require('fs');
const path = require('path');
const { EmailWaterfall } = require('../lib/email-waterfall');

const IN = path.join(__dirname, '..', 'output', 'us-immigration-lawyers-lawyerlegion.csv');
const OUT = path.join(__dirname, '..', 'output', 'lawyerlegion-waterfall-emails.csv');
...
async function main() {
  console.log('=== LawyerLegion Waterfall ===');
