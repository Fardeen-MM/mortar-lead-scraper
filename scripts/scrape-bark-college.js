#!/usr/bin/env node
/**
 * Bark.com College Consultants / Education Professionals Scraper
 *
 * Scrapes https://www.bark.com/en/{country}/{category}/{region}/{city}/
 * using Puppeteer (Bark blocks plain HTTP with 403).
 *
 * Categories tried per country:
 *   - tutoring
 *   - college-counselor
 ...
 *
 * Strategy:
 *   1. For each country + category slug, visit main category page to discover city URLs
 *   2. Combine all discovered city URLs across all categories (dedup by slug)
 *   3. For each city URL, collect company profile links
 *   4. Visit each profile page to extract: name, email, website, phone, location
 *   5. Dedup by firm_name + city
 *   6. Resume support via progress file
 *   7. Output CSV
 *
 * Usage:
 *   node scripts/scrape-bark-college.js --test                 # 3 cities, 3 profiles per city
 *   node scripts/scrape-bark-college.js --country UK           # All UK cities
 *   node scripts/scrape-bark-college.js --country US           # All US cities
 *   node scripts/scrape-bark-college.js --country CA           # All CA cities
 *   node scripts/scrape-bark-college.js --country ALL          # All countries
 *   node scripts/scrape-bark-college.js --country ALL --resume # Resume interrupted scrape
 */
...
const OUT_FILE = path.join(OUT_DIR, 'college-consultants-bark.csv');
const PROGRESS_FILE = path.join(OUT_DIR, 'bark-college-progress.json');
...
// Education category slugs to try on Bark
...
// mainUrls will be built from CATEGORY_SLUGS at discovery time
const COUNTRY_CONFIG = {
  UK: { code: 'gb', country: 'UK' },
  US: { code: 'us', country: 'US' },
  CA: { code: 'ca', country: 'CA' },
};
...
/**
 * Parse a firm/person name from the education/tutoring niche.
 * Strips education-specific business words to try to extract a person name.
 */
function parseName(firmName) {
  ...
  // "X of John Smith" pattern (e.g., "Tutoring Services of John Smith")
  const ofMatch = cleaned.match(/(?:(?:Tutoring|Education|Academic|Learning|Consulting|College\s+(?:Prep|Counseling|Consulting))\s+(?:Service|Services|Center|Centre|Group|Agency|Company|Studio|Hub|Lab|Academy|Institute|Solutions?)\s+of\s+)(.+)/i);
  if (ofMatch) {
    cleaned = ofMatch[1]
    ...
    cleaned = cleaned
      ...
      .replace(/,?\s*(LLC|PLLC|PC|PLC|PA|P\.A\.|LLP|Inc\.?|Ltd\.?|Limited|Co\.?)/gi, '')
      // Remove education-specific business words
      .replace(/\s*(Tutoring\s+(?:Service|Services|Center|Centre|Group|Agency|Company|Studio|Hub|Lab|Academy|Solutions?)|Tutor(?:ing)?s?\s+(?:Online|Near\s+Me|Pro|Plus)|College\s+(?:Prep|Counseling|Counselor|Consulting|Consultant|Advisors?|Admissions?|Planning|Coach(?:ing)?)|SAT(?:\/ACT)?\s+(?:Prep|Tutor(?:ing)?|Coach(?:ing)?)|ACT\s+(?:Prep|Tutor(?:ing)?|Coach(?:ing)?)|Test\s+Prep(?:aration)?|Academic\s+(?:Tutoring|Coaching|Support|Success|Excellence|Mentoring|Services?|Solutions?|Advisors?)|Educational?\s+(?:Consultant|Consulting|Services?|Solutions?|Advisors?|Coach(?:ing)?)|Learning\s+(?:Center|Centre|Hub|Lab|Academy|Studio|Solutions?|Services?)|Tutoring|Tutor|Prep(?:aration)?|Academy|Institute|Education(?:al)?|Consulting|Consultant|Counseling|Counselor|Coaching|Coach|Mentoring|Mentor|& Associates?|Associates?)/gi, '')
      .trim();
  }
  ...
  // Education-specific words that should not be treated as last names
  const eduWords = /^(tutoring|tutor|tutors|prep|academic|learning|education|educational|consultant|consulting|counseling|counselor|coaching|coach|college|admissions|academy|institute|mentoring|mentor|services?|solutions?|center|centre|hub|lab|studio|online|pro|plus|smart|elite|premier|global|first|bright|brilliant|star|genius|excel|success|advance|advanced|scholars?|scholars?|test|sat|act)$/i;

  // If what remains looks like a person name (2-3 words, starting with capitals)
  if (cleaned && /^[A-Z][a-z]+(\s+[A-Z]\.?)?\s+[A-Z][a-z]+(-[A-Z][a-z]+)?$/i.test(cleaned)) {
    const parts = cleaned.split(/\s+/);
    const lastName = parts[parts.length - 1];
    if (eduWords.test(lastName)) return { first_name: '', last_name: '' };
    if (eduWords.test(parts[0])) return { first_name: '', last_name: '' };
    return {
      first_name: parts[0],
      last_name: lastName,
    };
 ...
 * URL patterns:
 *   /en/gb/tutoring/london/                         -> city: London, state: ''
 *   /en/gb/tutoring/west-midlands/birmingham/        -> city: Birmingham, state: ''
 *   /en/us/college-counselor/california/los-angeles/  -> city: Los Angeles, state: CA
 *   /en/ca/tutoring/ontario/toronto/                  -> city: Toronto, state: ON
 */
function parseCityUrl(urlPath, countryKey, categorySlug) {
  const config = COUNTRY_CONFIG[countryKey];
  const basePrefix = `/en/${config.code}/${categorySlug}/`;
  ...
  if (parts.length === 2) {
    ...
    return { city, state, urlPath, slug: `${region}/${citySlug}`, categorySlug };
  } else if (parts.length === 1 && parts[0]) {
    // No region prefix: /city/
    const citySlug = parts[0];
    const city = slugToName(citySlug);
    return { city, state: '', urlPath, slug: citySlug, categorySlug };
  }
...
/**
 * Discover city URLs from the main listing page for a single category slug in a country.
 * Returns array of { city, state, url, slug, categorySlug }.
 */
async function discoverCityUrlsForCategory(page, countryKey, categorySlug) {
  const config = COUNTRY_CONFIG[countryKey];
  const mainUrl = `https://www.bark.com/en/${config.code}/${categorySlug}/`;
  log(`  Trying category "${categorySlug}" at ${mainUrl}`);

  try {
    const resp = await page.goto(mainUrl, { waitUntil: 'networkidle2', timeout: NAV_TIMEOUT });
    if (resp.status() === 404) {
      log(`    404 - category "${categorySlug}" not found for ${countryKey}`);
      return [];
    }
    if (resp.status() !== 200) {
      log(`    HTTP ${resp.status()} loading category page`);
      return [];
    }
  } catch (err) {
  ...
  const linkPaths = await page.evaluate((countryCode, catSlug) => {
    const paths = new Set();
    const basePrefix = `/en/${countryCode}/${catSlug}/`;
    document.querySelectorAll(`a[href*="${catSlug}"]`).forEach(a => {
  ...
  for (const linkPath of linkPaths) {
    const parsed = parseCityUrl(linkPath, countryKey, categorySlug);
    if (!parsed) continue;
  ...
  log(`    Found ${cities.length} city URLs for "${categorySlug}"`);
  return cities;
}

/**
 * Discover city URLs across ALL category slugs for a country.
 * Deduplicates by city slug so the same city is only scraped once
 * (using the first category that discovered it).
 */
async function discoverAllCityUrls(page, countryKey) {
  log(`Discovering cities for ${countryKey} across ${CATEGORY_SLUGS.length} categories`);

  const allCities = [];
  const seenSlugs = new Set();

  for (const categorySlug of CATEGORY_SLUGS) {
    const cities = await discoverCityUrlsForCategory(page, countryKey, categorySlug);
  ...
  log(`  Total unique city URLs for ${countryKey}: ${allCities.length}`);
  return allCities;
}

/**
 * Get list of company profile URLs from a city listing page.
 * Also follows pagination links to collect profiles from all pages.
 */
async function getProfileUrls(page, cityUrl) {
  ...
  while (currentUrl && pageNum <= maxPages) {
    try {
      const resp = await page.goto(currentUrl, { waitUntil: 'networkidle2', timeout: NAV_TIMEOUT });
      if (resp.status() === 404) {
        log(`  404 - no page`);
        break;
      }
    ...
    const { profileUrls, nextPageUrl } = await page.evaluate(() => {
      const links = new Set();
      ...
      if (nextLink) {
        ...
        if (href) {
          nextUrl = href.startsWith('http') ? href : `https://www.bark.com${href}`;
        }
      }

      return { profileUrls: [...links], nextPageUrl: nextUrl };
    });
  ...
  log(`  Found ${allProfileUrls.size} profile links (across ${pageNum} page${pageNum > 1 ? 's' : ''})`);
...
async function scrapeProfile(page, profileUrl, cityInfo, country) {
  ...
  const data = await page.evaluate(() => {
    ...
    // Phone — check for visible phone number
...
async function main() {
  ...
  if (!isTest && !countryArg) {
    console.log('Usage:');
    console.log('  node scripts/scrape-bark-college.js --test                 # Test mode (3 cities, 3 profiles/city)');
    console.log('  node scripts/scrape-bark-college.js --country UK           # All UK cities');
    console.log('  node scripts/scrape-bark-college.js --country US           # All US cities');
    console.log('  node scripts/scrape-bark-college.js --country CA           # All CA cities');
    console.log('  node scripts/scrape-bark-college.js --country ALL          # All countries');
    console.log('  node scripts/scrape-bark-college.js --country ALL --resume # Resume');
    process.exit(0);
  }
  ...
  log('Bark.com College Consultants / Education Scraper');
  log(`Categories: ${CATEGORY_SLUGS.join(', ')}`);
  ...
  for (const countryKey of countries) {
    ...
    // Step 1: Discover city URLs across all category slugs
    let cityJobs = await discoverAllCityUrls(page, countryKey);
    await randomDelay();
    ...
    for (let ci = 0; ci < citiesToScrape.length; ci++) {
      ...
      totalProcessed++;
      log(`\n[${ci + 1}/${citiesToScrape.length}] ${cityJob.city}, ${countryKey}${cityJob.state ? ' (' + cityJob.state + ')' : ''} [${cityJob.categorySlug}]`);
