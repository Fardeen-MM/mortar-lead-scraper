#!/usr/bin/env node
/**
 * Clutch.co Immigration Law Firms Scraper
 *
 * Scrapes https://clutch.co/law/immigration using Puppeteer + stealth plugin
 * (Cloudflare blocks plain HTTP requests).
 *
 * Strategy:
 *   1. Load listing pages (/law/immigration?page=N) with stealth Puppeteer
 ...
 *   5. Dedup by firm name + city
 *   6. Output CSV
 *
 * Usage:
 *   node scripts/scrape-clutch-immigration.js --test    # Page 1 only, first 5 profiles
 *   node scripts/scrape-clutch-immigration.js --all     # All pages + all profiles
 */
...
const BASE_URL = 'https://clutch.co/law/immigration';
...
const OUT_FILE = path.join(OUT_DIR, 'immigration-consulting-clutch.csv');
const PROGRESS_FILE = path.join(OUT_DIR, 'clutch-immigration-progress.json');
...
async function main() {
  ...
  if (!isTest && !isAll) {
    console.log('Usage:');
    console.log('  node scripts/scrape-clutch-immigration.js --test    # Page 1 only, first 5 profiles');
    console.log('  node scripts/scrape-clutch-immigration.js --all     # All pages + all profiles');
    process.exit(0);
  }

  if (!fs.existsSync(OUT_DIR)) fs.mkdirSync(OUT_DIR, { recursive: true });

  log('Clutch.co Immigration Law Firms Scraper');
  log(`Mode: ${isTest ? 'TEST (page 1, 5 profiles)' : 'ALL pages'}`);
  log(`Output: ${OUT_FILE}`);
  log('');
  ...
  try {
    // ── Phase 1: Collect listings from all pages ──

    log('Phase 1: Collecting listings from directory pages...');
    log('');

    // Load page 1 to discover total pages
    log(`Loading page 1: ${BASE_URL}`);
    ...
    log('');
    log(`Phase 1 complete: ${allListings.length} unique listings from ${totalListings} total`);
    log('');
