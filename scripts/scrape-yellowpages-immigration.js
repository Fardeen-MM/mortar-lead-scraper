#!/usr/bin/env node
/**
 * YellowPages.com Immigration Lawyers Scraper
 *
 * Scrapes https://www.yellowpages.com/search?search_terms=immigration+lawyer&geo_location_terms={City,ST}
 ...
 *   2. For each city, search for "immigration lawyer" and "immigration attorney"
 *   3. Extract business name, phone, address, website, categories from result cards
 *   4. Paginate through all result pages (30 per page, up to ~25 pages)
 ...
 *   - Rating (from .result-rating class)
 *
 * Usage:
 *   node scripts/scrape-yellowpages-immigration.js --test         # 5 cities
 *   node scripts/scrape-yellowpages-immigration.js --all-cities   # All 150+ cities
 *   node scripts/scrape-yellowpages-immigration.js --resume       # Resume interrupted scrape
 */
...
const OUT_FILE = path.join(OUT_DIR, 'us-immigration-lawyers-yellowpages.csv');
const PROGRESS_FILE = path.join(OUT_DIR, 'yellowpages-immigration-progress.json');

const CSV_COLUMNS = [
  'first_name', 'last_name', 'firm_name', 'title', 'email', 'phone',
  'website', 'domain', 'city', 'state', 'country', 'niche',
  'source', 'profile_url', 'address', 'categories',
];

// Search terms — run both to maximize coverage
const SEARCH_TERMS = [
  'immigration lawyer',
...
function parseName(firmName) {
  ...
  if (ofMatch) {
    ...
    cleaned = cleaned
      ...
      .replace(/\s*(Law\s+(?:Firm|Group|Office|Offices|Practice|Center|Corporation)|Legal\s+(?:Group|Services?|Team)|Immigration\s+(?:Law|Attorneys?|Lawyers?|Services?|Consulting)|Attorneys?\s+at\s+Law|Attorneys?|Lawyers?|& Associates?|Associates?|A Professional\s+.*)/gi, '')
      .trim();
  }
  ...
  const lawWords = /^(law|legal|firm|group|office|pllc|plc|pc|immigration|attorneys?|lawyers?|counselors?|associates?|consulting|services?)$/i;
...
function parseLocality(localityText) {
  // "New York, NY 10023" → { city: "New York", state: "NY", zip: "10023" }
...
async function getResultCount(page) {
  return await page.evaluate(() => {
    ...
    if (!el) return 0;
    // "Showing 1-30 of 743More info" → 743
...
async function main() {
  ...
  if (!isTest && !isAllCities && !isResume) {
    console.log('Usage:');
    console.log('  node scripts/scrape-yellowpages-immigration.js --test         # Test mode (5 cities)');
    console.log('  node scripts/scrape-yellowpages-immigration.js --all-cities   # All cities');
    console.log('  node scripts/scrape-yellowpages-immigration.js --resume       # Resume interrupted scrape');
    process.exit(0);
  }
  ...
  log(`YellowPages.com Immigration Lawyers Scraper`);
  ...
  async function waitForCloudflare(page, maxWait = CF_CHALLENGE_WAIT) {
    ...
    return false; // Still stuck on challenge
  }
  ...
  async function navigateWithCF(page, url) {
    for (let attempt = 0; attempt < CF_MAX_RETRIES; attempt++) {
      ...
      if (title.includes('moment') || title.includes('Cloudflare') || title.includes('Just a')) {
        ...
        return true; // Normal page loaded
      }
    }
    ...
    while (pageNum <= maxPagesPerSearch) {
      ...
      try {
        ...
        for (const r of results) {
          ...
          const address = [r.street, r.locality].filter(Boolean).join(', ');
