/**
 * Saskatchewan Law Society Scraper
 *
 * Source: https://lss.alinityapp.com/client/publicdirectory
 * Method: Alinity platform — SPA with underlying API (may have CAPTCHA)
 *
 * Alinity is a SaaS registration platform used by several Canadian law societies.
 * The public directory is typically an Angular/React SPA backed by a JSON API.
...
class SaskatchewanScraper extends BaseScraper {
  constructor() {
    super({
      name: 'saskatchewan',
      stateCode: 'CA-SK',
      baseUrl: 'https://lss.alinityapp.com/client/publicdirectory',
      pageSize: 50,
      practiceAreaCodes: {
        'family':                'Family',
        'family law':            'Family',
    ...
    this.apiBaseUrl = 'https://lss.alinityapp.com/api';
  }
  ...
  /**
   * Parse lawyers from Alinity HTML page (fallback when API is not available).
   */
  parseLawyersFromHtml(html) {
    const $ = cheerio.load(html);
    const attorneys = [];

    // Alinity renders a table or card-based directory
    $('table tbody tr, .directory-row, .registrant-row, .member-card').each((_, el) => {
      ...
      attorneys.push({
        ...
        profile_url: profileLink && profileLink.startsWith('http') ? profileLink : (profileLink ? `https://lss.alinityapp.com${profileLink}` : ''),
      });
    });
  ...
  /**
   * Async generator that yields attorney records from the SK Law Society.
    ...
    for (let ci = 0; ci < cities.length; ci++) {
      ...
      while (true) {
        ...
        if (useApi) {
          ...
          // Fall back to POST-based search if GET API did not work
...
module.exports = new SaskatchewanScraper();
