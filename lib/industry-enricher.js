...
function inferTitle(lead, niche) {
  ...
  if (/law\s*firm|lawyer|attorney|solicitor|barrister/i.test(nicheL)) return 'Attorney';
  if (/therapist|counselor|psycholog/i.test(nicheL)) return 'Therapist';
  if (/consult/i.test(nicheL)) return 'Consultant';
...
function enrichAll(leads, niche, options = {}) {
  ...
  for (let i = 0; i < leads.length; i++) {
    ...
    // Clean existing title — reject city names, state codes, and other junk
    if (lead.title) {
      const t = lead.title.trim();
      // Strip " - XX" state code suffix (e.g., "Attorney - AK")
      lead.title = t.replace(/\s*-\s*[A-Z]{2}$/, '').trim();
      // Reject titles that are clearly not titles (city names, numbers, single words < 3 chars)
      if (/^\d/.test(lead.title) || lead.title.length < 3 ||
          /^(north|south|east|west|mount|saint|fort|lake|new|san|los|las|el|la)\s/i.test(lead.title)) {
        lead.title = '';
      }
    }
