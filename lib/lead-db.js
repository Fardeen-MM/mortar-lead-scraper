...
function deleteLeads(leadIds) {
  ...
  const txn = db.transaction(() => {
    for (const id of leadIds) {
      // Clean up non-CASCADE dependent tables
      db.prepare("DELETE FROM lead_sources WHERE lead_id = ?").run(id);
      db.prepare("DELETE FROM lead_snapshots WHERE lead_id = ?").run(id);
      try { db.prepare("DELETE FROM sequence_enrollments WHERE lead_id = ?").run(id); } catch {}
      try { db.prepare("DELETE FROM lead_activities WHERE lead_id = ?").run(id); } catch {}
      try { db.prepare("DELETE FROM lead_territory WHERE lead_id = ?").run(id); } catch {}
      try { db.prepare("DELETE FROM campaign_leads WHERE lead_id = ?").run(id); } catch {}
      // CASCADE tables (list_members, lead_changelog, lead_score_history,
      // lead_notes, contact_log, enrichment_queue) are auto-deleted
...
function findPotentialDuplicates(limit = 100) {
  const db = getDb();
  // Split into 3 fast queries instead of one slow OR-joined self-join
  const half = Math.ceil(limit / 3);
  const emailDupes = db.prepare(`
    SELECT a.id as id1, b.id as id2,
           ...
           'email' as match_type
    FROM leads a JOIN leads b ON a.id < b.id AND LOWER(a.email) = LOWER(b.email)
    WHERE a.email IS NOT NULL AND a.email != '' LIMIT ?
  `).all(half);
  const phoneDupes = db.prepare(`
    SELECT a.id as id1, b.id as id2,
           ...
           b.email as email2, b.phone as phone2, b.firm_name as firm2,
           'phone' as match_type
    FROM leads a JOIN leads b ON a.id < b.id AND a.phone = b.phone
    WHERE a.phone IS NOT NULL AND a.phone != '' AND length(a.phone) >= 7 LIMIT ?
  `).all(half);
  const nameDupes = db.prepare(`
    SELECT a.id as id1, b.id as id2,
           ...
           b.first_name as fn2, b.last_name as ln2, b.city as city2, b.state as state2,
           b.email as email2, b.phone as phone2, b.firm_name as firm2,
           'name+city' as match_type
    FROM leads a JOIN leads b ON a.id < b.id
      AND a.last_name = b.last_name AND a.first_name = b.first_name
      AND a.city = b.city AND a.state = b.state
    WHERE a.last_name != '' AND a.first_name != '' LIMIT ?
  `).all(half);
  // Dedupe by id pair and combine
  const seen = new Set();
  const results = [];
  for (const d of [...emailDupes, ...phoneDupes, ...nameDupes]) {
  ...
  const txn = db.transaction(() => {
    for (const group of groups) {
      ...
      if (dryRun) {
        ...
        for (const tbl of ['lead_notes', 'lead_activities', 'lead_changelog', 'lead_snapshots', 'list_members']) {
          try { db.prepare(`DELETE FROM ${tbl} WHERE lead_id = ?`).run(discard.id); } catch {}
        }
...
// --- Merge Candidates (Batch 34) --- (Optimized: subquery + indexed columns)
function getMergeCandidates(limit = 30) {
  const db = getDb();
  // Same name + city — optimized with GROUP BY first, then join back
  const nameDupes = db.prepare(`
    WITH dupes AS (
      SELECT LOWER(first_name) as fn, LOWER(last_name) as ln, LOWER(city) as ct, MIN(id) as min_id
      FROM leads WHERE city IS NOT NULL AND city != '' AND first_name IS NOT NULL AND last_name IS NOT NULL
      GROUP BY LOWER(first_name), LOWER(last_name), LOWER(city)
      HAVING COUNT(*) > 1
      LIMIT ?
    )
    ...
    FROM dupes d
    JOIN leads l1 ON LOWER(l1.first_name) = d.fn AND LOWER(l1.last_name) = d.ln AND LOWER(l1.city) = d.ct
    JOIN leads l2 ON LOWER(l2.first_name) = d.fn AND LOWER(l2.last_name) = d.ln AND LOWER(l2.city) = d.ct AND l2.id > l1.id
    LIMIT ?
  `).all(limit, limit * 2);
  // Same email (non-empty) — optimized with GROUP BY
  const emailDupes = db.prepare(`
    WITH dup_emails AS (
      SELECT LOWER(email) as em, MIN(id) as min_id
      FROM leads WHERE email IS NOT NULL AND email != ''
      GROUP BY LOWER(email) HAVING COUNT(*) > 1
      LIMIT ?
    )
    SELECT l1.id as id1, l2.id as id2,
      ...
      'email' as match_type
    FROM dup_emails d
    JOIN leads l1 ON LOWER(l1.email) = d.em
    JOIN leads l2 ON LOWER(l2.email) = d.em AND l2.id > l1.id
    LIMIT ?
  `).all(limit, limit * 2);
