...
/**
 * Normalize phone to last 10 digits for fast indexed dedup.
 */
...
function getDb() {
  ...
  // Backfill phone_normalized for existing leads (one-time migration)
  try {
    const needsBackfill = _db.prepare("SELECT COUNT(*) as c FROM leads WHERE phone != '' AND phone IS NOT NULL AND (phone_normalized = '' OR phone_normalized IS NULL)").get();
    if (needsBackfill.c > 0) {
      const toFix = _db.prepare("SELECT id, phone FROM leads WHERE phone != '' AND phone IS NOT NULL AND (phone_normalized = '' OR phone_normalized IS NULL)").all();
      const update = _db.prepare("UPDATE leads SET phone_normalized = ? WHERE id = ?");
      const txn = _db.transaction(() => {
        for (const row of toFix) {
          const digits = (row.phone || '').replace(/\D/g, '');
          const norm = digits.length >= 7 ? digits.slice(-10) : '';
          update.run(norm, row.id);
        }
      });
      txn();
      console.log(`[lead-db] Backfilled phone_normalized for ${toFix.length} leads`);
    }
