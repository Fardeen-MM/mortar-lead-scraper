...
/**
 * Normalize phone for indexed dedup lookup.
 * Uses the same logic as normalizer.js to strip country codes,
 * then takes last 10 digits for consistent matching.
 */
function _normalizePhone(phone) {
  ...
  // Take last 10 digits — covers US/CA (10), UK (10), AU (9), SG (8), HK (8)
...
function getDb() {
  ...
  addColIfMissing('enrichment_attempts', "INTEGER DEFAULT 0"); // how many times waterfall was attempted
  addColIfMissing('last_enrichment_error', "TEXT DEFAULT ''"); // last error from profile fetch
  ...
  addColIfMissing('phone_normalized', "TEXT DEFAULT ''"); // last 10 digits of phone for fast dedup

  // Sync phone_normalized for all leads with phones (uses normalizer for country code stripping)
  try {
    const phonesToSync = _db.prepare("SELECT id, phone FROM leads WHERE phone != '' AND phone IS NOT NULL").all();
    let synced = 0;
    if (phonesToSync.length > 0) {
      const update = _db.prepare("UPDATE leads SET phone_normalized = ? WHERE id = ? AND (phone_normalized != ? OR phone_normalized IS NULL)");
      const txn = _db.transaction(() => {
        for (const row of phonesToSync) {
          const norm = _normalizePhone(row.phone);
          const r = update.run(norm, row.id, norm);
          if (r.changes > 0) synced++;
        }
      });
      txn();
      if (synced > 0) console.log(`[lead-db] Synced phone_normalized for ${synced} leads`);
    }
  } catch (e) { /* ignore — column may not exist yet in older DBs */ }

  // === Data quality cleanup migrations (idempotent) ===
  try {
    // 1. Clear invalid phones: "--", "-", or fewer than 7 digits
    const badPhones = _db.prepare("SELECT id, phone FROM leads WHERE phone != '' AND phone IS NOT NULL").all()
      .filter(r => {
        const digits = (r.phone || '').replace(/\D/g, '');
        return digits.length < 7;
      });
    if (badPhones.length > 0) {
      const clearPhone = _db.prepare("UPDATE leads SET phone = '', phone_normalized = '' WHERE id = ?");
      const txn = _db.transaction(() => {
        for (const r of badPhones) clearPhone.run(r.id);
      });
      txn();
      console.log(`[lead-db] Cleaned ${badPhones.length} invalid phone numbers`);
    }

    // 2. Fix websites missing protocol (www.example.com → https://www.example.com)
    const noProtocol = _db.prepare("SELECT id, website FROM leads WHERE website LIKE 'www.%'").all();
    if (noProtocol.length > 0) {
      const fixUrl = _db.prepare("UPDATE leads SET website = 'https://' || website WHERE id = ?");
      const txn = _db.transaction(() => {
        for (const r of noProtocol) fixUrl.run(r.id);
      });
      txn();
      console.log(`[lead-db] Fixed ${noProtocol.length} URLs missing protocol`);
    }

    // 3. Fix PEI numeric bar status codes
    ...
    const fixStatus = _db.prepare("UPDATE leads SET bar_status = ? WHERE bar_status = ?");
    const statusTxn = _db.transaction(() => {
      for (const [code, label] of Object.entries(numericStatuses)) {
        fixStatus.run(label, code);
      }
    });
    statusTxn();

    // 4. Fix FindLaw firm names stored as last_name: move to firm_name, clear last_name
    const firmInLastName = _db.prepare(
      "SELECT id, last_name FROM leads WHERE first_name = '' AND last_name != '' AND firm_name = '' AND primary_source = 'findlaw_bar'"
    ).all();
    if (firmInLastName.length > 0) {
      const fixFirm = _db.prepare("UPDATE leads SET firm_name = last_name, last_name = '' WHERE id = ?");
      const firmTxn = _db.transaction(() => {
        for (const r of firmInLastName) fixFirm.run(r.id);
      });
      firmTxn();
      console.log(`[lead-db] Fixed ${firmInLastName.length} FindLaw firm-as-last-name records`);
    }
  } catch (e) {
    console.error('[lead-db] Data cleanup migration error:', e.message);
  }

  // Now that optional columns exist, create indexes on them
  _db.exec(`
...
function findExistingLead(lead) {
  ...
  // 4. Phone match (indexed lookup on phone_normalized — last 10 digits)
  if (lead.phone) {
...
function upsertLead(lead) {
  ...
  if (existingId) {
    ...
    if (updates.phone) {
      updates.phone_normalized = _normalizePhone(updates.phone);
    }
  ...
  // Insert new lead
  const phoneNorm = _normalizePhone(lead.phone);
...
function getStats() {
  ...
  const byPracticeArea = db.prepare(
    "SELECT practice_area, COUNT(*) as count FROM leads WHERE practice_area IS NOT NULL AND practice_area != '' GROUP BY practice_area ORDER BY count DESC LIMIT 20"
  ).all();

  // Enrichment stats
  const enriched = db.prepare(
    "SELECT COUNT(*) as count FROM leads WHERE last_enriched_at IS NOT NULL"
  ).get().count;
  const enrichedLast24h = db.prepare(
    "SELECT COUNT(*) as count FROM leads WHERE last_enriched_at > datetime('now', '-1 day')"
  ).get().count;
  const withProfileUrl = db.prepare(
    "SELECT COUNT(*) as count FROM leads WHERE profile_url IS NOT NULL AND profile_url != ''"
  ).get().count;
  const withLinkedin = db.prepare(
    "SELECT COUNT(*) as count FROM leads WHERE linkedin_url IS NOT NULL AND linkedin_url != ''"
  ).get().count;
  const withTitle = db.prepare(
    "SELECT COUNT(*) as count FROM leads WHERE title IS NOT NULL AND title != ''"
  ).get().count;
...
function searchLeads(query, options = {}) {
  const db = getDb();
  const { state, country, hasEmail, hasPhone, hasWebsite, practiceArea, minScore, maxScore, tags, source, sort, order, limit = 100, offset = 0, enrichedAfter, hasProfileUrl, hasLinkedin, hasEnrichmentError, createdAfter } = options;
  ...
  if (enrichedAfter) {
    where.push("last_enriched_at >= ?");
    params.push(enrichedAfter);
  }
  if (hasProfileUrl) {
    where.push("profile_url != '' AND profile_url IS NOT NULL");
  }
  if (hasLinkedin) {
    where.push("linkedin_url != '' AND linkedin_url IS NOT NULL");
  }
  if (hasEnrichmentError) {
    where.push("last_enrichment_error != '' AND last_enrichment_error IS NOT NULL");
  }
  if (createdAfter) {
  ...
  const sortableCols = { name: 'last_name', first_name: 'first_name', firm: 'firm_name', city: 'city', state: 'state', score: 'lead_score', email: 'email', phone: 'phone', source: 'primary_source', updated: 'updated_at', created: 'created_at', enriched: 'last_enriched_at' };
...
function exportLeads(options = {}) {
  const db = getDb();
  const { state, country, hasEmail, hasPhone, hasWebsite, verified, enrichedAfter, practiceArea, tags, minScore } = options;
  ...
  if (hasWebsite) { where.push("website != '' AND website IS NOT NULL"); }
  if (verified) { where.push('email_verified = 1'); }
  if (enrichedAfter) { where.push('last_enriched_at >= ?'); params.push(enrichedAfter); }
  if (practiceArea) { where.push('practice_area LIKE ?'); params.push(`%${practiceArea}%`); }
  if (tags) { where.push('tags LIKE ?'); params.push(`%${tags}%`); }
  if (minScore !== undefined && minScore !== null) { where.push('lead_score >= ?'); params.push(Number(minScore)); }
...
function getStateDetails(stateCode) {
  ...
  try {
    ...
    `).all(stateCode);
  } catch(e) { /* scrape_runs table may not exist */ }
 ...
 */
const FK_TABLES = ['lead_notes', 'lead_activities', 'lead_changelog', 'lead_snapshots', 'list_members', 'lead_sources', 'quality_alerts', 'sequence_enrollments', 'lead_territory', 'campaign_leads', 'contact_log', 'enrichment_queue'];

function safeDeleteLead(db, id) {
  for (const tbl of FK_TABLES) {
    try { db.prepare(`DELETE FROM ${tbl} WHERE lead_id = ?`).run(id); } catch {}
  }
...
function updateLead(id, updates) {
  ...
  const allowedFields = [
    ...
    'phone', 'email', 'website', 'bar_number', 'bar_status', 'admission_date',
    'practice_area', 'title', 'linkedin_url', 'bio', 'education', 'languages',
    'tags', 'notes', 'email_source', 'phone_source', 'website_source',
    'practice_specialties', 'rating', 'google_place_id',
    'last_enriched_at', 'enrichment_steps',
  ...
  if (sets.length === 0) return { updated: false };
  // Keep phone_normalized in sync
  if (updates.phone !== undefined) {
    sets.push("phone_normalized = ?");
    values.push(_normalizePhone(updates.phone));
  }
...
function findPotentialDuplicates(limit = 100) {
  ...
  // Use GROUP BY instead of self-joins (200x faster on 13K+ rows)
  const emailDupes = db.prepare(`
    SELECT LOWER(email) as match_key, GROUP_CONCAT(id) as ids, COUNT(*) as cnt, 'email' as match_type
    FROM leads WHERE email IS NOT NULL AND email != ''
    GROUP BY LOWER(email) HAVING cnt >= 2
    ORDER BY cnt DESC LIMIT ?
  `).all(third);
  const phoneDupes = db.prepare(`
    SELECT phone as match_key, GROUP_CONCAT(id) as ids, COUNT(*) as cnt, 'phone' as match_type
    FROM leads WHERE phone IS NOT NULL AND phone != '' AND LENGTH(phone) >= 7
    GROUP BY phone HAVING cnt >= 2
    ORDER BY cnt DESC LIMIT ?
  `).all(third);
  const nameDupes = db.prepare(`
    SELECT LOWER(first_name)||'|'||LOWER(last_name)||'|'||LOWER(city)||'|'||state as match_key,
      GROUP_CONCAT(id) as ids, COUNT(*) as cnt, 'name+city' as match_type
    FROM leads WHERE first_name != '' AND last_name != '' AND city IS NOT NULL AND city != ''
    GROUP BY match_key HAVING cnt >= 2
    ORDER BY cnt DESC LIMIT ?
  `).all(third);
  // Convert groups to pairs for backward compatibility
  const results = [];
  const seen = new Set();
  for (const group of [...emailDupes, ...phoneDupes, ...nameDupes]) {
    const ids = group.ids.split(',').map(Number);
    if (ids.length < 2) continue;
    // Load lead data for first two in group
    const leads = db.prepare(`SELECT id, first_name, last_name, city, state, email, phone, firm_name FROM leads WHERE id IN (${ids.slice(0,2).join(',')})`).all();
    ...
    results.push({
      id1: a.id, id2: b.id,
      fn1: a.first_name, ln1: a.last_name, city1: a.city, state1: a.state,
      email1: a.email, phone1: a.phone, firm1: a.firm_name,
      fn2: b.first_name, ln2: b.last_name, city2: b.city, state2: b.state,
      email2: b.email, phone2: b.phone, firm2: b.firm_name,
...
function mergeLeadPair(keepId, deleteId) {
  ...
  const txn = db.transaction(() => {
    ...
    // Delete the duplicate (clean FK references first)
    safeDeleteLead(db, deleteId);
  });
  txn();
...
function autoMergeDuplicatesLegacy(confidenceThreshold = 90) {
  const db = getDb();
...
function getEnrichmentStats() {
  ...
  // Per-state enrichment gaps — shows ROI opportunity per state
  const stateGaps = db.prepare(`
    SELECT state,
      ...
      SUM(CASE WHEN profile_url IS NOT NULL AND profile_url != '' THEN 1 ELSE 0 END) as has_profile_url,
  ...
  const withErrors = db.prepare("SELECT COUNT(*) as cnt FROM leads WHERE last_enrichment_error IS NOT NULL AND last_enrichment_error != ''").get().cnt;
  const avgAttempts = db.prepare("SELECT AVG(enrichment_attempts) as avg FROM leads WHERE enrichment_attempts > 0").get().avg || 0;
  ...
  return {
    ...
    stateGaps,
    errors: { withErrors, avgAttempts: Math.round(avgAttempts * 10) / 10 },
  };
}
...
function findSimilarLeads(leadId, limit = 10) {
  ...
  const excludeIds = sameFirm.map(l => l.id);
  ...
  `).all(lead.city, lead.state, leadId, ...excludeIds) : [];
...
function initNotesTable() {
  ...
  `);
  // Add pinned column if table was created by older schema
  try { db.exec('ALTER TABLE lead_notes ADD COLUMN pinned INTEGER DEFAULT 0'); } catch(e) {}
}
...
function mergeLeadsWithChoices(keepId, mergeId, fieldChoices = {}) {
  ...
  // Delete the merged lead (clean FK references first)
  safeDeleteLead(db, mergeId);
...
function previewSmartListCount(filters) {
  return executeSmartListQuery(filters, true);
}

function updateSmartList(id, updates) {
  const db = getDb();
  ensureSmartListTable();
  const sets = [];
  const params = [];
  ...
  if (updates.filters !== undefined) {
    ...
    const count = executeSmartListQuery(updates.filters, true);
    sets.push('last_count = ?');
    params.push(count);
  }
  if (sets.length === 0) return;
  params.push(id);
  return db.prepare(`UPDATE smart_lists SET ${sets.join(', ')} WHERE id = ?`).run(...params);
}

function getSmartListStats() {
  const db = getDb();
  ensureSmartListTable();
  const lists = db.prepare("SELECT * FROM smart_lists ORDER BY created_at DESC").all();
  ...
  let totalLeads = 0;
  for (const list of lists) {
    totalLeads += list.last_count || 0;
  }
  return { total, totalLeads, lists };
}
...
function mergeLeadsWithPicks(targetId, sourceIds, fieldPicks = {}) {
  ...
  // Delete source leads (clean FK references first)
  const deleteIds = sourceIds.filter(id => id !== targetId);
  for (const delId of deleteIds) {
    safeDeleteLead(db, delId);
  }
...
function scanForDuplicates(limit = 200) {
  ...
  const emailGroups = db.prepare(`
    SELECT GROUP_CONCAT(id) as ids FROM leads
    WHERE email IS NOT NULL AND email != ''
    GROUP BY LOWER(email) HAVING COUNT(*) >= 2 LIMIT ?
  `).all(limit);
  for (const g of emailGroups) {
    const ids = g.ids.split(',').map(Number).sort((a,b) => a-b);
    for (let i = 0; i < ids.length - 1; i++) {
      try { db.prepare("INSERT INTO dedup_queue (lead_id_a, lead_id_b, match_type, confidence) VALUES (?, ?, 'email', 95)").run(ids[i], ids[i+1]); found++; } catch {}
    }
  }

  const nameGroups = db.prepare(`
    SELECT GROUP_CONCAT(id) as ids FROM leads
    WHERE first_name IS NOT NULL AND last_name IS NOT NULL AND city IS NOT NULL AND city != ''
    GROUP BY LOWER(first_name), LOWER(last_name), LOWER(city) HAVING COUNT(*) >= 2 LIMIT ?
  `).all(limit);
  for (const g of nameGroups) {
    const ids = g.ids.split(',').map(Number).sort((a,b) => a-b);
    for (let i = 0; i < ids.length - 1; i++) {
      try { db.prepare("INSERT INTO dedup_queue (lead_id_a, lead_id_b, match_type, confidence) VALUES (?, ?, 'name_city', 75)").run(ids[i], ids[i+1]); found++; } catch {}
    }
  }
  ...
  const phoneGroups = db.prepare(`
    ...
    GROUP BY phone HAVING COUNT(*) >= 2 LIMIT ?
  `).all(limit);
  for (const g of phoneGroups) {
    const ids = g.ids.split(',').map(Number).sort((a,b) => a-b);
    for (let i = 0; i < ids.length - 1; i++) {
      try { db.prepare("INSERT INTO dedup_queue (lead_id_a, lead_id_b, match_type, confidence) VALUES (?, ?, 'phone', 90)").run(ids[i], ids[i+1]); found++; } catch {}
    }
  }
...
function getPracticeAreaAnalytics(limit = 30) {
  ...
  `).all();
  // Cross-sell opportunities: firms spanning multiple practice areas
  // Use GROUP_CONCAT + JS parsing instead of O(n²) self-join
  const multiPaFirms = db.prepare(`
    SELECT firm_name, GROUP_CONCAT(DISTINCT practice_area) as practices, COUNT(DISTINCT practice_area) as pa_count
    FROM leads WHERE firm_name IS NOT NULL AND firm_name != '' AND practice_area IS NOT NULL AND practice_area != ''
    GROUP BY LOWER(firm_name) HAVING pa_count >= 2
  `).all();
  const pairCounts = {};
  for (const f of multiPaFirms) {
    const pas = f.practices.split(',');
    for (let i = 0; i < pas.length; i++) {
      for (let j = i + 1; j < pas.length; j++) {
        const key = [pas[i], pas[j]].sort().join('|');
        pairCounts[key] = (pairCounts[key] || 0) + 1;
      }
    }
  }
  const crossSell = Object.entries(pairCounts)
    .filter(([,c]) => c >= 2)
    .sort((a,b) => b[1] - a[1])
    .slice(0, 20)
    .map(([key, count]) => { const [a,b] = key.split('|'); return { area_a: a, area_b: b, shared_firms: count }; });
...
function getNetworkMap(limit = 30) {
  ...
  const connectors = db.prepare(`
    WITH firm_stats AS (
      SELECT LOWER(firm_name) as firm_key, COUNT(DISTINCT city) as firm_cities, COUNT(*) as firm_size,
        MIN(id) as rep_id
      FROM leads WHERE firm_name IS NOT NULL AND firm_name != ''
      GROUP BY firm_key HAVING firm_cities >= 2
      ORDER BY firm_cities DESC, firm_size DESC LIMIT 15
    )
    SELECT l.id, l.first_name, l.last_name, l.firm_name, l.city, l.state, l.email, l.lead_score,
      fs.firm_cities, fs.firm_size
    FROM firm_stats fs JOIN leads l ON l.id = fs.rep_id
  `).all();
...
// --- Merge Candidates (Batch 34) --- (Optimized: simple GROUP BY + no self-join)
function getMergeCandidates(limit = 30) {
  const db = getDb();
  // Same name + city — return groups, not pairs (much faster)
  const nameDupes = db.prepare(`
    SELECT first_name, last_name, city, state, COUNT(*) as count,
      GROUP_CONCAT(id) as ids,
      GROUP_CONCAT(email, ' | ') as emails,
      GROUP_CONCAT(phone, ' | ') as phones,
      GROUP_CONCAT(firm_name, ' | ') as firms
    FROM leads
    WHERE city IS NOT NULL AND city != '' AND first_name IS NOT NULL AND last_name IS NOT NULL
    GROUP BY LOWER(first_name), LOWER(last_name), LOWER(city)
    HAVING COUNT(*) > 1
    ORDER BY COUNT(*) DESC
    LIMIT ?
...
function getRelationshipGraph(limit = 30) {
  const db = getDb();
  // Practice area clusters (fast — no self-join)
  ...
  `).all();
  // Multi-city firms (fast — group by instead of self-join)
  const firmBridges = db.prepare(`
    SELECT firm_name, COUNT(DISTINCT city) as city_count, COUNT(*) as people,
      GROUP_CONCAT(DISTINCT city) as cities
    FROM leads WHERE firm_name IS NOT NULL AND firm_name != ''
      AND city IS NOT NULL AND city != ''
    GROUP BY LOWER(firm_name)
    HAVING city_count >= 2
    ORDER BY city_count DESC, people DESC LIMIT ?
  `).all(limit);
  // Top city connections by shared firms (fast — pre-aggregated)
  const cityPairs = db.prepare(`
    WITH firm_cities AS (
      SELECT LOWER(firm_name) as fn, city
      FROM leads WHERE firm_name IS NOT NULL AND firm_name != ''
        AND city IS NOT NULL AND city != ''
      GROUP BY LOWER(firm_name), city
    )
    SELECT a.city as city1, b.city as city2, COUNT(*) as shared_firms
    FROM firm_cities a JOIN firm_cities b ON a.fn = b.fn AND a.city < b.city
    GROUP BY a.city, b.city HAVING shared_firms >= 2
...
function getRevenueAttribution() {
  const db = getDb();
  const practiceValues = { 'Personal Injury': 5000, 'Medical Malpractice': 8000, 'Corporate': 10000, 'Real Estate': 4000, 'Criminal Defense': 3000, 'Family Law': 3000, 'Bankruptcy': 2500, 'Immigration': 2000, 'Employment': 4000, 'IP': 7000, 'Tax': 6000, 'International Law': 8000 };
  // Revenue by source
  const bySource = db.prepare(`
    SELECT primary_source, COUNT(*) as leads,
      SUM(CASE WHEN email IS NOT NULL AND email != '' THEN 1 ELSE 0 END) as contactable,
      ROUND(AVG(lead_score), 1) as avg_score
    FROM leads WHERE primary_source IS NOT NULL
    GROUP BY primary_source ORDER BY leads DESC
  `).all();
  const sourceROI = bySource.map(s => ({
    ...s,
    estimated_value: s.contactable * 50, // $50 per contactable lead
    roi_score: Math.round((s.contactable / Math.max(s.leads, 1)) * s.avg_score)
  }));
  // Revenue by geography
  const byGeo = db.prepare(`
    SELECT state, city, COUNT(*) as leads,
      SUM(CASE WHEN email IS NOT NULL AND email != '' THEN 1 ELSE 0 END) as contactable,
      ROUND(AVG(lead_score), 1) as avg_score
    FROM leads WHERE state IS NOT NULL
    GROUP BY state, city ORDER BY contactable DESC LIMIT 20
  `).all();
  // High-value segments
  const segments = db.prepare(`
    SELECT practice_area, COUNT(*) as leads,
      SUM(CASE WHEN email IS NOT NULL AND email != '' THEN 1 ELSE 0 END) as contactable
    FROM leads WHERE practice_area IS NOT NULL AND practice_area != ''
    GROUP BY practice_area ORDER BY leads DESC LIMIT 15
  `).all().map(s => ({
    ...s,
    value_per_lead: practiceValues[s.practice_area] || 3000,
    total_value: s.contactable * (practiceValues[s.practice_area] || 3000)
  }));
  return { sourceROI, byGeo, segments };
}

// --- Saturation Heatmap (Batch 39) ---
function getSaturationHeatmap() {
  const db = getDb();
  // City market size estimates (rough approximations)
  const cityEstimates = { 'New York': 80000, 'Los Angeles': 40000, 'Chicago': 30000, 'Houston': 20000, 'Miami': 15000, 'San Francisco': 12000, 'Dallas': 12000, 'Philadelphia': 10000, 'Phoenix': 8000, 'Austin': 6000, 'Denver': 7000, 'Seattle': 8000, 'Boston': 15000, 'Atlanta': 10000, 'London': 50000, 'Sydney': 20000, 'Toronto': 15000, 'Melbourne': 12000 };
  const cityData = db.prepare(`
    SELECT city, state, COUNT(*) as our_leads,
      SUM(CASE WHEN email IS NOT NULL AND email != '' THEN 1 ELSE 0 END) as with_email,
      ROUND(AVG(lead_score), 1) as avg_score
    FROM leads WHERE city IS NOT NULL AND city != ''
    GROUP BY city, state ORDER BY our_leads DESC LIMIT 30
  `).all();
  const heatmap = cityData.map(c => ({
    ...c,
    estimated_market: cityEstimates[c.city] || 5000,
    penetration: Math.round((c.our_leads / (cityEstimates[c.city] || 5000)) * 10000) / 100,
    saturation: c.our_leads > (cityEstimates[c.city] || 5000) * 0.1 ? 'high' : c.our_leads > (cityEstimates[c.city] || 5000) * 0.02 ? 'medium' : 'low'
  }));
  // Practice area penetration
  const practiceData = db.prepare(`
    SELECT practice_area, COUNT(*) as leads, COUNT(DISTINCT city) as cities
    FROM leads WHERE practice_area IS NOT NULL AND practice_area != ''
    GROUP BY practice_area ORDER BY leads DESC LIMIT 15
  `).all();
  return { heatmap, practiceData };
}

// --- Smart List Builder (Batch 39) ---
function getSmartListBuilder() {
  const db = getDb();
  // Pre-built list templates
  const templates = [];
  templates.push({
    name: 'High-Value Ready',
    description: 'Leads with email + phone + high score',
    count: db.prepare("SELECT COUNT(*) as c FROM leads WHERE email IS NOT NULL AND email != '' AND phone IS NOT NULL AND phone != '' AND lead_score >= 50").get().c
  });
  templates.push({
    name: 'Ready to Contact',
    description: 'Has email, never contacted',
    count: db.prepare("SELECT COUNT(*) as c FROM leads WHERE email IS NOT NULL AND email != '' AND pipeline_stage = 'new'").get().c
  });
  templates.push({
    name: 'Needs Enrichment',
    description: 'Has name but no email or phone',
    count: db.prepare("SELECT COUNT(*) as c FROM leads WHERE (email IS NULL OR email = '') AND (phone IS NULL OR phone = '')").get().c
  });
  templates.push({
    name: 'Has Website No Email',
    description: 'Website crawl candidates',
    count: db.prepare("SELECT COUNT(*) as c FROM leads WHERE website IS NOT NULL AND website != '' AND (email IS NULL OR email = '')").get().c
  });
  templates.push({
    name: 'Top Firms',
    description: 'Leads from firms with 5+ attorneys',
    count: db.prepare("SELECT COUNT(*) as c FROM leads WHERE LOWER(firm_name) IN (SELECT LOWER(firm_name) FROM leads WHERE firm_name IS NOT NULL AND firm_name != '' GROUP BY LOWER(firm_name) HAVING COUNT(*) >= 5)").get().c
  });
  templates.push({
    name: 'Big City Leads',
    description: 'Leads in tier-1 cities',
    count: db.prepare("SELECT COUNT(*) as c FROM leads WHERE city IN ('New York','Los Angeles','Chicago','Houston','Miami','San Francisco','London','Sydney','Toronto')").get().c
  });
  // Filter preview counts
  const filterCounts = {
    withEmail: db.prepare("SELECT COUNT(*) as c FROM leads WHERE email IS NOT NULL AND email != ''").get().c,
    withPhone: db.prepare("SELECT COUNT(*) as c FROM leads WHERE phone IS NOT NULL AND phone != ''").get().c,
    withWebsite: db.prepare("SELECT COUNT(*) as c FROM leads WHERE website IS NOT NULL AND website != ''").get().c,
    withLinkedin: db.prepare("SELECT COUNT(*) as c FROM leads WHERE linkedin_url IS NOT NULL AND linkedin_url != ''").get().c,
    total: db.prepare("SELECT COUNT(*) as c FROM leads").get().c
  };
  return { templates, filterCounts };
}

// --- Quality Scorecard (Batch 39) ---
function getQualityScorecard() {
  const db = getDb();
  const sources = db.prepare(`
    SELECT primary_source, COUNT(*) as total,
      ROUND(100.0 * SUM(CASE WHEN email IS NOT NULL AND email != '' THEN 1 ELSE 0 END) / COUNT(*), 1) as email_pct,
      ROUND(100.0 * SUM(CASE WHEN phone IS NOT NULL AND phone != '' THEN 1 ELSE 0 END) / COUNT(*), 1) as phone_pct,
      ROUND(100.0 * SUM(CASE WHEN website IS NOT NULL AND website != '' THEN 1 ELSE 0 END) / COUNT(*), 1) as website_pct,
      ROUND(100.0 * SUM(CASE WHEN firm_name IS NOT NULL AND firm_name != '' THEN 1 ELSE 0 END) / COUNT(*), 1) as firm_pct,
      ROUND(AVG(lead_score), 1) as avg_score,
      ROUND(AVG(JULIANDAY('now') - JULIANDAY(COALESCE(updated_at, created_at))), 0) as avg_age_days
    FROM leads WHERE primary_source IS NOT NULL
    GROUP BY primary_source ORDER BY total DESC
  `).all();
  // Grade each source
  const graded = sources.map(s => {
    let score = 0;
    score += Math.min(s.email_pct, 100) * 0.3;
    score += Math.min(s.phone_pct, 100) * 0.25;
    score += Math.min(s.website_pct, 100) * 0.15;
    score += Math.min(s.firm_pct, 100) * 0.15;
    score += Math.max(0, 100 - s.avg_age_days) * 0.15;
    const grade = score >= 80 ? 'A' : score >= 60 ? 'B' : score >= 40 ? 'C' : score >= 20 ? 'D' : 'F';
    return { ...s, quality_score: Math.round(score), grade };
  });
  // Overall database grade
  const overall = db.prepare(`
    SELECT COUNT(*) as total,
      ROUND(100.0 * SUM(CASE WHEN email IS NOT NULL AND email != '' THEN 1 ELSE 0 END) / COUNT(*), 1) as email_pct,
      ROUND(100.0 * SUM(CASE WHEN phone IS NOT NULL AND phone != '' THEN 1 ELSE 0 END) / COUNT(*), 1) as phone_pct,
    ...
    FROM leads
  `).get();
  let overallScore = (overall.email_pct * 0.4) + (overall.phone_pct * 0.3) + (overall.avg_score * 0.3);
  const overallGrade = overallScore >= 80 ? 'A' : overallScore >= 60 ? 'B' : overallScore >= 40 ? 'C' : overallScore >= 20 ? 'D' : 'F';
  return { sources: graded, overall: { ...overall, quality_score: Math.round(overallScore), grade: overallGrade } };
}

// --- Dedup Intelligence (Batch 40) ---
function getDedupIntelligence(limit = 25) {
  const db = getDb();
  // Exact name+city dupes
  const exactDupes = db.prepare(`
    SELECT LOWER(first_name) || '|' || LOWER(last_name) || '|' || LOWER(city) as key,
      COUNT(*) as count, GROUP_CONCAT(id) as ids,
      MIN(first_name) as first_name, MIN(last_name) as last_name, MIN(city) as city
    FROM leads WHERE first_name IS NOT NULL AND last_name IS NOT NULL AND city IS NOT NULL
  ...
  const emailDomains = db.prepare(`
    SELECT SUBSTR(email, INSTR(email, '@') + 1) as domain, COUNT(*) as count,
      GROUP_CONCAT(DISTINCT city) as cities
    FROM leads WHERE email IS NOT NULL AND email != '' AND INSTR(email, '@') > 0
  ...
  // Summary stats
  const stats = {
    exactDupeGroups: exactDupes.length,
    totalDupeLeads: exactDupes.reduce((s, d) => s + d.count, 0),
    emailDomainClusters: emailDomains.length,
    phoneDupeGroups: phoneDupes.length,
    estimatedSavings: exactDupes.reduce((s, d) => s + d.count - 1, 0)
  };
  return { exactDupes, emailDomains, phoneDupes, stats };
}

// --- Outbound Readiness (Batch 40) ---
function getOutboundReadiness() {
  const db = getDb();
  const readiness = db.prepare(`
    SELECT
      COUNT(*) as total,
      SUM(CASE WHEN email IS NOT NULL AND email != '' AND first_name IS NOT NULL THEN 1 ELSE 0 END) as email_ready,
      SUM(CASE WHEN phone IS NOT NULL AND phone != '' AND first_name IS NOT NULL THEN 1 ELSE 0 END) as phone_ready,
      SUM(CASE WHEN email IS NOT NULL AND email != '' AND phone IS NOT NULL AND phone != '' THEN 1 ELSE 0 END) as multi_channel,
      SUM(CASE WHEN (email IS NULL OR email = '') AND (phone IS NULL OR phone = '') THEN 1 ELSE 0 END) as no_contact_info,
      SUM(CASE WHEN website IS NOT NULL AND website != '' AND (email IS NULL OR email = '') THEN 1 ELSE 0 END) as crawl_candidates,
      SUM(CASE WHEN firm_name IS NOT NULL AND firm_name != '' AND (email IS NULL OR email = '') THEN 1 ELSE 0 END) as firm_search_candidates
    FROM leads
  `).get();
  // Readiness by state
  const byState = db.prepare(`
    SELECT state, COUNT(*) as total,
      SUM(CASE WHEN email IS NOT NULL AND email != '' THEN 1 ELSE 0 END) as email_ready,
      SUM(CASE WHEN phone IS NOT NULL AND phone != '' THEN 1 ELSE 0 END) as phone_ready
    FROM leads WHERE state IS NOT NULL
    GROUP BY state ORDER BY email_ready DESC LIMIT 15
  `).all();
  // Action items
  const actions = [];
  if (readiness.crawl_candidates > 0) actions.push({ action: 'Crawl firm websites', potential: readiness.crawl_candidates, channel: 'email' });
  if (readiness.no_contact_info > 0) actions.push({ action: 'Cross-reference directories', potential: Math.round(readiness.no_contact_info * 0.3), channel: 'both' });
  if (readiness.firm_search_candidates > 0) actions.push({ action: 'Firm website lookup', potential: Math.round(readiness.firm_search_candidates * 0.2), channel: 'email' });
  // Readiness score
  const score = Math.round(((readiness.email_ready + readiness.phone_ready) / Math.max(readiness.total * 2, 1)) * 100);
  return { readiness, byState, actions, score };
}
...
function getAgingReport() {
  ...
  const categories = db.prepare(`
    SELECT
      CASE
        WHEN JULIANDAY('now') - JULIANDAY(COALESCE(updated_at, created_at)) <= 7 THEN 'fresh_7d'
        WHEN JULIANDAY('now') - JULIANDAY(COALESCE(updated_at, created_at)) <= 30 THEN 'warm_30d'
        WHEN JULIANDAY('now') - JULIANDAY(COALESCE(updated_at, created_at)) <= 60 THEN 'cold_60d'
        ELSE 'stale_60d+'
      END as age_category,
      COUNT(*) as count,
      ROUND(AVG(lead_score), 1) as avg_score,
      SUM(CASE WHEN email IS NOT NULL AND email != '' THEN 1 ELSE 0 END) as with_email
    FROM leads GROUP BY age_category
  `).all();
  // Aging by source
  const bySource = db.prepare(`
    SELECT primary_source,
      ROUND(AVG(JULIANDAY('now') - JULIANDAY(COALESCE(updated_at, created_at))), 1) as avg_age,
      COUNT(*) as total,
      SUM(CASE WHEN JULIANDAY('now') - JULIANDAY(COALESCE(updated_at, created_at)) > 60 THEN 1 ELSE 0 END) as stale
    FROM leads WHERE primary_source IS NOT NULL
    GROUP BY primary_source ORDER BY avg_age DESC LIMIT 20
  `).all();
  // Re-engagement suggestions
  const suggestions = [];
  const staleWithEmail = db.prepare("SELECT COUNT(*) as c FROM leads WHERE JULIANDAY('now') - JULIANDAY(COALESCE(updated_at, created_at)) > 30 AND email IS NOT NULL AND email != ''").get().c;
  if (staleWithEmail > 0) suggestions.push({ action: 'Re-engage via email', count: staleWithEmail, priority: 'high' });
  const staleWithPhone = db.prepare("SELECT COUNT(*) as c FROM leads WHERE JULIANDAY('now') - JULIANDAY(COALESCE(updated_at, created_at)) > 30 AND phone IS NOT NULL AND phone != ''").get().c;
  if (staleWithPhone > 0) suggestions.push({ action: 'Re-engage via phone', count: staleWithPhone, priority: 'medium' });
  return { categories, bySource, suggestions };
}
...
function getGrowthAnalytics() {
  ...
  const recentRate = dailyGrowth.length > 0 ? Math.round(dailyGrowth.reduce((s, d) => s + d.new_leads, 0) / Math.max(dailyGrowth.length, 1)) : 0;
  const projections = {
    current: totals.total,
    dailyRate: recentRate,
    to20k: recentRate > 0 ? Math.ceil((20000 - totals.total) / recentRate) : null,
    to50k: recentRate > 0 ? Math.ceil((50000 - totals.total) / recentRate) : null,
    to100k: recentRate > 0 ? Math.ceil((100000 - totals.total) / recentRate) : null
  };
  // Quality trend
  ...
  `).all();
  return { weeklyGrowth, dailyGrowth, projections, qualityTrend };
}

// --- Waterfall Run History (Batch 41) ---

function _ensureWaterfallRuns() {
  const db = getDb();
  db.exec(`CREATE TABLE IF NOT EXISTS waterfall_runs (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    state_filter TEXT,
    ...
    profiles_attempted INTEGER DEFAULT 0,
  ...
  const addCol = (col, type) => {
    try { db.prepare(`SELECT ${col} FROM waterfall_runs LIMIT 1`).get(); }
    catch { db.exec(`ALTER TABLE waterfall_runs ADD COLUMN ${col} ${type}`); }
  };
  addCol('profiles_attempted', 'INTEGER DEFAULT 0');
  addCol('profiles_failed', 'INTEGER DEFAULT 0');
}

function recordWaterfallRun(data) {
  const db = getDb();
  _ensureWaterfallRuns();
  return db.prepare(`
    INSERT INTO waterfall_runs (state_filter, leads_processed, leads_enriched, fields_filled,
      profiles_fetched, profiles_attempted, profiles_failed,
      websites_found, emails_found, trigger, stats, completed_at, duration_secs)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), ?)
  `).run(
    data.stateFilter || '',
    data.leadsProcessed || 0,
    data.leadsEnriched || 0,
    data.fieldsFilled || 0,
    data.profilesFetched || 0,
    data.profilesAttempted || 0,
    data.profilesFailed || 0,
    data.websitesFound || 0,
    data.emailsFound || 0,
    data.trigger || 'manual',
...
function getWaterfallRuns(limit = 20) {
  const db = getDb();
  _ensureWaterfallRuns();
  return db.prepare('SELECT * FROM waterfall_runs ORDER BY id DESC LIMIT ?').all(limit);
}

function getWaterfallSummary() {
  const db = getDb();
  _ensureWaterfallRuns();
  const total = db.prepare('SELECT COUNT(*) as cnt FROM waterfall_runs').get().cnt;
  const allTime = db.prepare(`
    SELECT COALESCE(SUM(leads_enriched), 0) as total_enriched,
           COALESCE(SUM(fields_filled), 0) as total_fields,
           COALESCE(SUM(profiles_fetched), 0) as total_profiles,
           COALESCE(SUM(websites_found), 0) as total_websites,
           COALESCE(SUM(emails_found), 0) as total_emails,
           COALESCE(ROUND(AVG(duration_secs), 1), 0) as avg_duration
    FROM waterfall_runs
  `).get();
  const last = db.prepare('SELECT * FROM waterfall_runs ORDER BY id DESC LIMIT 1').get();
  return { totalRuns: total, ...allTime, lastRun: last || null };
}

// --- Enrichment Failure Tracking (Batch 42) ---

function getEnrichmentFailures(state, limit = 100) {
  const db = getDb();
  let sql = `SELECT id, first_name, last_name, firm_name, city, state, profile_url,
    enrichment_attempts, last_enrichment_error, last_enriched_at
    FROM leads WHERE last_enrichment_error != '' AND last_enrichment_error IS NOT NULL`;
  const params = [];
  if (state) { sql += ' AND state = ?'; params.push(state); }
  sql += ' ORDER BY enrichment_attempts DESC, updated_at DESC LIMIT ?';
  params.push(limit);
  return db.prepare(sql).all(...params);
}

function getEnrichmentFailureStats() {
  const db = getDb();
  // Count leads by error type
  const byError = db.prepare(`
    SELECT last_enrichment_error as error, COUNT(*) as cnt
    FROM leads WHERE last_enrichment_error != '' AND last_enrichment_error IS NOT NULL
    GROUP BY last_enrichment_error ORDER BY cnt DESC LIMIT 20
  `).all();
  // Count by state
  const byState = db.prepare(`
  ...
  const total = db.prepare(`
    SELECT COUNT(*) as total,
      SUM(CASE WHEN enrichment_attempts >= 3 THEN 1 ELSE 0 END) as exhausted
    FROM leads WHERE last_enrichment_error != '' AND last_enrichment_error IS NOT NULL
  `).get();
  return { total: total.total, exhausted: total.exhausted, byError, byState };
}

function clearEnrichmentErrors(state) {
  const db = getDb();
  let sql = "UPDATE leads SET last_enrichment_error = '', enrichment_attempts = 0 WHERE last_enrichment_error != ''";
  const params = [];
  if (state) { sql += ' AND state = ?'; params.push(state); }
...
function getTodayDigest() {
  ...
  // Top state today
  const topState = db.prepare(`
    SELECT state, COUNT(*) as count FROM leads
    WHERE created_at >= ? AND state IS NOT NULL AND state != ''
    GROUP BY state ORDER BY count DESC LIMIT 1
  `).get(today);

  // Recent scrape runs today
  const recentRuns = db.prepare(`
    SELECT state, leads_found, leads_new, emails_found, completed_at
    FROM scrape_runs WHERE completed_at >= ? ORDER BY completed_at DESC LIMIT 5
  `).all(today);

  // Enrichment activity today
  const enrichToday = db.prepare(`
    SELECT COUNT(*) as count FROM leads
    WHERE last_enriched_at >= ?
  `).get(today);

  // Score improvements (leads scored higher today)
  const highScoreToday = db.prepare(`
    SELECT COUNT(*) as count FROM leads
    WHERE created_at >= ? AND lead_score >= 60
  `).get(today);

  return {
    today: {
      added: todayStats.added,
      withEmail: todayStats.withEmail || 0,
      withPhone: todayStats.withPhone || 0,
      enriched: enrichToday.count,
      highScore: highScoreToday.count,
    },
    yesterday: { added: yesterdayStats.added },
    topState: topState || null,
    recentRuns,
    trend: todayStats.added > yesterdayStats.added ? 'up' : todayStats.added < yesterdayStats.added ? 'down' : 'flat',
  };
}
...
module.exports = {
  ...
  // Saturation heatmap (Batch 39)
  getSaturationHeatmap,
  // Smart list builder (Batch 39)
  getSmartListBuilder,
  // Quality scorecard (Batch 39)
  getQualityScorecard,
  // Dedup intelligence (Batch 40)
  getDedupIntelligence,
  // Outbound readiness (Batch 40)
  ...
  getEnrichmentFailureStats,
  clearEnrichmentErrors,
  getTodayDigest,
};
