...
function searchLeads(query, options = {}) {
  const db = getDb();
  const { state, country, hasEmail, hasPhone, hasWebsite, practiceArea, minScore, maxScore, tags, source, sort, order, limit = 100, offset = 0, enrichedAfter, hasProfileUrl } = options;
...
function recordWaterfallRun(data) {
  ...
  return db.prepare(`
    INSERT INTO waterfall_runs (state_filter, leads_processed, leads_enriched, fields_filled,
      profiles_fetched, websites_found, emails_found, trigger, stats, completed_at, duration_secs)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), ?)
  `).run(
    data.stateFilter || '',
