...
const commonCrawl = require('./commoncrawl-client');
const { extractDomain, normalizeTrade } = require('./normalizer');
const { enrichAll: industryEnrich } = require('./industry-enricher');
...
function runPipeline(options = {}) {
  ...
  (async () => {
    ...
    try {
      for await (const result of scraper.search(options.practice, searchOptions)) {
        ...
        const lead = {
          ...
          trade_category: normalizeTrade(result.practice_area || result.license_type || ''),
    ...
    // Step 4b: Supplementary discovery for industry scrapes (DuckDuckGo)
    if (!isLawyerScrape && newLeads.length > 0 && options.city && !cancelled) {
      try {
        log.info('Running supplementary DuckDuckGo discovery...');
        const nicheQuery = options.niche || 'business';
        const locationQuery = options.city || '';
        const ddgResults = await ddg.search(nicheQuery, locationQuery, {
          maxResults: options.test ? 20 : 50,
        });
        ...
        for (const result of ddgResults) {
          const ddgLead = {
            ...
            practice_specialties: '',
            email_source: '',
            phone_source: result.phone ? 'ddg' : '',
            website_source: result.website ? 'ddg' : '',
          };

          const { isDuplicate } = deduper.check(ddgLead);
          if (!isDuplicate) {
            deduper.addToKnown(ddgLead);
            newLeads.push(ddgLead);
            ddgAdded++;
            emitter.emit('lead', { data: ddgLead });
          }
        }
        log.info(`DuckDuckGo: added ${ddgAdded} supplementary leads`);
        stats.ddgAdded = ddgAdded;
      } catch (err) {
        log.warn(`DuckDuckGo supplementary search failed: ${err.message}`);
      }
    }

    // Step 4c: Common Crawl email enrichment for industry scrapes
    if (!isLawyerScrape && newLeads.length > 0 && !cancelled) {
      const leadsWithoutEmail = newLeads.filter(l => !l.email && l.website);
      if (leadsWithoutEmail.length > 0) {
        const domains = [...new Set(leadsWithoutEmail.map(l => extractDomain(l.website)).filter(Boolean))];
        const limit = options.test ? Math.min(5, domains.length) : Math.min(30, domains.length);
        if (limit > 0) {
          log.info(`Searching Common Crawl for emails on ${limit} domains...`);
          try {
            const ccEmails = await commonCrawl.batchFindEmails(domains.slice(0, limit));
            let ccApplied = 0;
            for (const lead of leadsWithoutEmail) {
              const domain = extractDomain(lead.website);
              if (domain && ccEmails.has(domain)) {
                const emails = ccEmails.get(domain);
                if (emails.length > 0) {
                  lead.email = emails[0];
            ...
            log.info(`Common Crawl: ${ccApplied} emails applied`);
            stats.ccEmailsApplied = ccApplied;
          } catch (err) {
            log.warn(`Common Crawl search failed: ${err.message}`);
          }
        }
      }
    }

    // Step 4d: Industry enrichment (DM scoring, title inference, LinkedIn, email patterns)
    if (!isLawyerScrape && newLeads.length > 0 && !cancelled) {
      log.info(`Running industry enrichment for ${newLeads.length} leads...`);
      const industryStats = industryEnrich(newLeads, options.niche || '', {
        onProgress: (i, total) => {
          if (i % 50 === 0 || i === total) {
            emitter.emit('progress', {
              totalScraped: stats.totalScraped,
      ...
      stats.industryEnrich = industryStats;
      stats.emailsFound = newLeads.filter(l => l.email).length;
      log.info(`Industry enrichment: ${industryStats.titlesInferred} titles, ${industryStats.emailPatternsGenerated} email patterns, ${industryStats.linkedInBuilt} LinkedIn`);
    }
    ...
    if (waterfallEnabled && newLeads.length > 0) {
      ...
      stats.waterfall = { totalFieldsFilled: 0, dbLookups: 0, profilesFetched: 0, crossRefMatches: 0, emailsCrawled: 0, profileFieldsFilled: 0, crossRefFieldsFilled: 0 };
      try {
    ...
    if (newLeads.length > 0) {
      ...
      try {
        ...
        emitter.emit('error', { message: `CSV write failed: ${err.message}` });
      }
    } else {
