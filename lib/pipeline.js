...
function runPipeline(options = {}) {
  ...
  (async () => {
    ...
    const waterfallEnabled = waterfallOpts.fetchProfiles !== false ||
    ...
    if (waterfallEnabled && newLeads.length > 0) {
      ...
      try {
        const waterfallStats = await runWaterfall(newLeads, {
          fetchProfiles: waterfallOpts.fetchProfiles !== false,
          crossRefMartindale: waterfallOpts.crossRefMartindale !== false,
          crossRefLawyersCom: waterfallOpts.crossRefLawyersCom !== false,
          nameLookups: waterfallOpts.nameLookups !== false,
        ...
        if (cancelled) {
          ...
          log.success(
            ...
            `(${waterfallStats.profilesFetched} profiles, ${waterfallStats.crossRefMatches} cross-refs, ` +
            `${waterfallStats.emailsCrawled} emails crawled)`
          );
        }
      } catch (err) {
