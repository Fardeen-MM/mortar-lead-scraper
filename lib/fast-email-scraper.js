 ...
 */
async function scrapeEmails(domain, maxPages = 4) {
  const allEmails = new Set();
  const baseUrl = `https://${domain}`;
  let homepageHtml = '';

  // Phase 1: Try predefined contact paths
  for (let i = 0; i < Math.min(maxPages, CONTACT_PATHS.length); i++) {
    ...
    try {
      ...
      if (i === 0) homepageHtml = html;

      // If we found domain-matching emails, we're good
      if (found.some(e => e.endsWith('@' + domain)) && found.length >= 2) break;
    } catch {
      // Skip failed pages
    }
  }

  // Phase 2: If still no domain emails, discover contact links from homepage
  // (kaymen99 pattern: find links containing "contact" and scrape those)
  if (![...allEmails].some(e => e.endsWith('@' + domain)) && homepageHtml) {
    const linkRe = /href=["']([^"']*contact[^"']*?)["']/gi;
    ...
    while ((m = linkRe.exec(homepageHtml)) !== null) {
      ...
      if (href.includes(domain)) contactLinks.add(href);
    }
