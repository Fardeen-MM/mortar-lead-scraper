...
class EmailWaterfall {
  ...
  async findEmail(lead) {
    ...
    // Normalize Unicode (strip accents: Muñoz→munoz, René→rene) then lowercase
    const f = first_name.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().replace(/[^a-z]/g, '');
    const l = last_name.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().replace(/[^a-z]/g, '');
  ...
  _allPatterns(f, l, domain) {
    const fi = f[0];
    const li = l[0];
    // Ordered by most common in corporate environments (from Fortune 500 analysis)
    return [
      `${f}.${l}@${domain}`,      // first.last@ (48% of companies)
      `${fi}${l}@${domain}`,      // flast@ (very common, Deloitte etc.)
      `${f}${l}@${domain}`,       // firstlast@
      `${f}@${domain}`,           // first@
      `${fi}.${l}@${domain}`,     // f.last@
      `${f}_${l}@${domain}`,      // first_last@
      `${l}.${f}@${domain}`,      // last.first@ (common in EU/Asia)
      `${l}@${domain}`,           // last@
      ...
      `${fi}_${l}@${domain}`,     // f_last@ (common in mid-size firms)
      `${fi}-${l}@${domain}`,     // f-last@ (UK firms)
      `${l}_${f}@${domain}`,      // last_first@
      `${l}-${f}@${domain}`,      // last-first@
      `${fi}${li}@${domain}`,     // fl@ (rare but exists)
    ];
  }
  ...
  async _verifyMicrosoft(candidates, f, l, domain) {
    // Try known pattern first, then top 6 candidates
    ...
    for (const email of toCheck) {
      try {
        ...
        throttleCount = 0; // Reset on success
      } catch (err) {
        if (err.message === 'throttled') {
          throttleCount++;
          // Exponential backoff: 2s, 4s, 8s
          await this._sleep(2000 * Math.pow(2, throttleCount - 1));
          if (throttleCount >= 3) break; // Give up, fall through to Autodiscover
        }
        // Other errors — continue to next candidate
      }
      // Delay between checks to avoid triggering throttle
      await this._sleep(300);
    }

    // If none verified via MS GetCredentialType, try Autodiscover.
    // NOTE (audit 2026-04-16): HTTP 200 from Autodiscover is NOT user-exists proof —
    // it means "domain is federated/recognized" and returns data for ANY
    // user at a federated tenant. Demoted to "probable" confidence 55.
    // Only 302→mailbox-config redirects are treated as higher confidence.
    for (const email of toCheck.slice(0, 3)) {
      try {
        const ad = await this._msAutodiscover(email);
        if (ad && ad.exists === true) {
          return {
            email,
            source: 'microsoft_autodiscover',
            status: ad.confidence >= 80 ? 'verified' : 'probable',
      ...
      const req = https.request({
        ...
        res.on('end', () => {
          try {
            ...
            // Check throttle status FIRST — when throttled, results are unreliable
            // ThrottleStatus: 0 = NotThrottled, 1 = AadThrottled, 2 = MsaThrottled
            if (j.ThrottleStatus && j.ThrottleStatus !== 0) {
              reject(new Error('throttled'));
              return;
            }
            // IfExistsResult: 0 = exists, 1 = doesn't exist, 2 = throttled,
            // 4 = error, 5 = other tenant, 6 = on-prem ADFS
            if (j.IfExistsResult === 2) { reject(new Error('throttled')); return; }
            if (j.IfExistsResult === 4 || j.IfExistsResult === -1) { reject(new Error('ms_error')); return; }
  ...
  async _verifyGoogle(candidates, f, l, domain) {
    ...
    // 4. Don't guess — return null and let caller decide
    // (45% confidence was polluting results with unverified emails)
    return null;
  }
  ...
  _smtpCheck(mxHost, email) {
    return new Promise(resolve => {
      ...
      socket.on('data', (data) => {
        ...
        if (phase === 'connect' && response.includes('220')) {
        ...
        } else if (phase === 'ehlo' && /^250 /m.test(response)) {
          // Wait for complete EHLO response (line starting with "250 " not "250-")
          phase = 'mail';
          response = '';
          socket.write('MAIL FROM:<verify@mortar.app>\r\n'); // Use realistic sender, not null
          ...
          else if (/^5\d{2}/m.test(response)) { finish('invalid'); } // Proper 5xx status code check
          else { finish('unknown'); }
        }
      });
      ...
      try {
        ...
        if (exists) {
          return {
            ...
            // Gravatar adoption has declined in 2026 — many B2B users never signed up.
            // Presence is a weak signal that email was valid AT SOME POINT, not currently.
            // Demoted from 65 → 50 per audit 2026-04-16.
            confidence: 50,
          };
        }
      } catch {}
  ...
  // GET the autodiscover JSON endpoint
  // NOTE: Semantics are complex — 200 can mean federated domain (not user confirmation),
  // 302 can redirect to actual mailbox (user exists). We treat 200 as likely-exists
  // and 302 to a non-error URL as likely-exists too. Lower confidence than GetCredentialType.

  /**
   * Microsoft Autodiscover check.
   *
   * FIX (audit 2026-04-16): previous code returned `true` for HTTP 200, but
   * 200 only means "the domain is federated/recognized" — for federated
   * tenants the endpoint returns 200 for ANY address at that domain,
   * generating false positives at 92% confidence. Now returns a tiered result.
   *
   * Returns: { exists: bool|null, confidence: 0-90 }
   *   - exists=true confidence=85: 302 redirect with Location to mailbox config
   *   - exists=true confidence=55: 200 with valid response body (federated ambiguity)
   *   - exists=false: 302 to error/notfound, or 404
   *   - exists=null: network error / timeout
   */
        ...
        (res) => {
          ...
          res.on('end', () => {
            if (res.statusCode === 302) {
              ...
              if (loc.includes('error') || loc.includes('notfound') || loc.includes('user-not-found')) {
                resolve({ exists: false, confidence: 0 });
              } else if (loc.includes('autodiscover') || loc.includes('outlook.office365') || loc.includes('autodiscover-s.outlook')) {
                // Redirected to a real mailbox autoconfig endpoint — user likely exists
                resolve({ exists: true, confidence: 85 });
              } else {
                // Unknown redirect — treat as ambiguous
                resolve({ exists: true, confidence: 50 });
              }
            } else if (res.statusCode === 200) {
              // 200 means domain is federated/recognized — NOT user-exists proof.
              // Check if response body contains protocol info (server=, DisplayName=, etc.)
              // which is a weak signal that the user's mailbox was resolved.
              let hasProtocol = false;
              try {
                const j = JSON.parse(body);
                hasProtocol = !!(j && j.Protocol && (j.Protocol.Server || j.Protocol.Url));
              } catch {}
              // Federated tenants return 200 for any user — low confidence.
              resolve({ exists: true, confidence: hasProtocol ? 60 : 45 });
            } else if (res.statusCode === 404 || res.statusCode === 400) {
              resolve({ exists: false, confidence: 0 });
            } else {
              resolve({ exists: null, confidence: 0 });
            }
          });
        }
      );
      req.on('error', () => resolve({ exists: null, confidence: 0 }));
      req.on('timeout', () => { req.destroy(); resolve({ exists: null, confidence: 0 }); });
    });
  }
  ...
  async verifyEmail(email) {
    ...
    if (mx.provider === 'microsoft') {
      ...
      result = { valid: grav ? true : null, reason: grav ? 'gravatar_confirmed' : 'unverifiable_google', confidence: grav ? 50 : 0 };
    } else {
      // SMTP check
      if (!this.catchAllCache.has(domain)) {
