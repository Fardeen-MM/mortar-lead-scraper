...
class EmailWaterfall {
      ...
      try {
        ...
        if (exists) {
          return {
            ...
            confidence: 65, // Gravatar = registered somewhere, not proof email is currently active
          };
        }
      } catch {}
    }
    ...
    return new Promise((resolve) => {
      const req = https.get(
        ...
        (res) => {
          res.on('data', () => {});
          res.on('end', () => {
            // 200 = domain recognized, likely exists (but could be federated redirect)
            // 302 to error page = doesn't exist, 302 to mailbox = exists
            if (res.statusCode === 200) resolve(true);
            else if (res.statusCode === 302) {
              const loc = res.headers.location || '';
              // If redirected to an error/not-found page, user doesn't exist
              if (loc.includes('error') || loc.includes('notfound')) resolve(false);
              // If redirected to a mailbox config URL, user likely exists
              else resolve(true);
            }
            else resolve(false);
          });
  ...
  async verifyEmail(email) {
    ...
    if (mx.provider === 'microsoft') {
      ...
      result = { valid: grav ? true : null, reason: grav ? 'gravatar_confirmed' : 'unverifiable_google', confidence: grav ? 65 : 0 };
    } else {
      // SMTP check
      if (!this.catchAllCache.has(domain)) {
