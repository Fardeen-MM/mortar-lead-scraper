...
class BulkScraper extends EventEmitter {
  ...
  async run(options = {}) {
    ...
    // Auto-enrich after bulk scrape
    if (this.progress.totalNew > 0) {
      try {
        console.log('[Bulk] Running post-scrape enrichment...');
        ...
        this.progress.enrichment = {
          merged: mergeResult.merged,
          firmShared: firmResult.leadsUpdated,
          websitesDeduced: deduceResult.leadsUpdated,
        };
        console.log(`[Bulk] Post-enrichment: ${mergeResult.merged} dupes merged, ${firmResult.leadsUpdated} firm shares, ${deduceResult.leadsUpdated} websites deduced`);
      } catch (err) {
        console.error('[Bulk] Post-enrichment error:', err.message);
      }
    }
  ...
  _runSingleScraper(code, options = {}) {
    return new Promise((resolve, reject) => {
      ...
      // Timeout per scraper (5 min in test mode, 2 hours in full mode)
      const timeout = options.test ? 5 * 60 * 1000 : 2 * 60 * 60 * 1000;
      setTimeout(() => {
        emitter.emit('cancel');
