...
/**
 * Build Ideal Customer Profile from best leads
 */
async function buildICP(topLeads, allStats) {
  const system = `You are an ICP (Ideal Customer Profile) analyst for a legal marketing agency. Given a sample of top-performing leads and overall database statistics, define the ideal customer profile.

Return JSON:
{
  "icp_summary": "2-3 sentence description of ideal customer",
  "attributes": [
    {
      "dimension": "Practice Area|Geography|Firm Size|Seniority|Data Quality",
      "ideal_value": "what the ideal looks like",
      "weight": "high|medium|low",
      "reasoning": "why this matters"
    }
  ],
  "scoring_criteria": [
    { "factor": "factor name", "points": number, "condition": "when to award" }
  ],
  "anti_patterns": ["characteristics of leads that WON'T convert"],
  "recommendations": ["3-5 actionable recommendations for lead sourcing"]
}`;

  const leadSummaries = topLeads.slice(0, 20).map(l => ({
    name: `${l.first_name || ''} ${l.last_name || ''}`.trim(),
    firm: l.firm_name || '',
    practice: l.practice_area || '',
    city: l.city || '', state: l.state || '',
    score: l.lead_score || 0,
    has_email: !!l.email, has_phone: !!l.phone,
    has_website: !!l.website, has_linkedin: !!l.linkedin_url,
    title: l.title || '',
  }));

  const prompt = `Analyze these top leads and database stats to build an ICP:

Top Leads (highest scoring):
${JSON.stringify(leadSummaries, null, 1)}

Database Stats:
${JSON.stringify(allStats, null, 1)}

Build a detailed ICP profile with scoring criteria.`;
...
/**
 * AI-powered source analysis and recommendations
 */
async function analyzeSourceROI(sourceData) {
  const system = `You are a data source ROI analyst. Given scraping source performance data, recommend which sources to prioritize, expand, or drop.
  ...
  "rankings": [
    {
      "source": "source name",
      "grade": "A|B|C|D|F",
      "strengths": "what this source does well",
      "weaknesses": "what's missing",
      "recommendation": "expand|maintain|deprioritize"
    }
  ],
  "strategy": ["3-5 strategic recommendations for data sourcing"]
}`;

  const prompt = `Analyze these lead data source metrics:
${JSON.stringify(sourceData, null, 1)}

Rank each source and provide actionable recommendations.`;
