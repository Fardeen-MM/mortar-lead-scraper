...
class RateLimiter {
  constructor(options = {}) {
    ...
    this.totalBlocks = 0;
    this.maxTotalBlocks = options.maxTotalBlocks || 10;
  ...
  async handleBlock(statusCode) {
    ...
    // Give up after too many total blocks
    if (this.totalBlocks >= this.maxTotalBlocks) {
      log.error(`${this.totalBlocks} total blocks — giving up`);
      return false;
    }
