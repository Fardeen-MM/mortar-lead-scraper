 ...
 *   0. Master DB cross-reference (instant SQLite lookup from previous scrapes)
 ...
 * @param {boolean} [options.masterDbLookup=true] - Cross-ref master lead DB
...
async function runWaterfall(leads, options = {}) {
  const {
    ...
    nameLookups = true,
    emailCrawl = true,
    smtpPatterns, // defaults to emailCrawl value if not set
    emitter,
    isCancelled = () => false,
    emailFinder,
    domainCache,
  } = options;
  const doSmtp = smtpPatterns !== undefined ? smtpPatterns : emailCrawl;
  ...
  const stats = {
    ...
    profilesAttempted: 0,
    profilesFailed: 0,
    profileErrors: [], // [{leadId, name, error}]
  ...
  // --- Step 0: Master DB Cross-Reference (instant SQLite lookup) ---
  if (masterDbLookup) {
    const leadsNeedingData = leads.filter(l => needsEnrichment(l));
    if (leadsNeedingData.length > 0) {
      log.info(`Waterfall Step 0: Cross-referencing ${leadsNeedingData.length} leads against master DB`);
      emitProgress('master-db', 0, leadsNeedingData.length, 'Checking master database');

      try {
        const leadDb = require('./lead-db');
        const dbResults = leadDb.batchLookupByNameCity(leadsNeedingData);

        let enriched = 0;
        for (const [idx, data] of dbResults) {
          // Don't use primary_source in the merge — it's metadata, not a field to copy
          const { primary_source, ...enrichData } = data;
          const sourceName = primary_source ? `db:${primary_source}` : 'master-db';
          const filled = mergeFields(leadsNeedingData[idx], enrichData, sourceName);
          if (filled.length > 0) {
            enriched++;
            stats.dbFieldsFilled += filled.length;
            stats.totalFieldsFilled += filled.length;
          }
        }

        stats.dbLookups = enriched;
        log.success(`Step 0 done: ${enriched} leads enriched from master DB, ${stats.dbFieldsFilled} fields filled`);
      } catch (err) {
        log.warn(`Master DB lookup failed: ${err.message}`);
      }

      emitProgress('master-db', leadsNeedingData.length, leadsNeedingData.length, 'Done');
    }
  }
  ...
  if (fetchProfiles) {
    ...
    if (leadsWithProfiles.length > 0) {
      ...
      for (const [source, sourceLeads] of bySource) {
        ...
        // Fetch profiles with concurrency (default 3 for speed, configurable)
        const concurrency = options.profileConcurrency || 3;
        let i = 0;
        while (i < sourceLeads.length) {
          if (isCancelled()) break;
          const batch = sourceLeads.slice(i, i + concurrency);
          ...
          for (let j = 0; j < batch.length; j++) {
            if (results[j].status === 'rejected') {
              ...
              const errMsg = results[j].reason?.message || String(results[j].reason || 'Unknown error');
              const leadName = `${batch[j].first_name} ${batch[j].last_name}`;
              log.warn(`Profile fetch failed for ${leadName}: ${errMsg}`);
              stats.profileErrors.push({
                leadId: batch[j].id,
                name: leadName,
                error: errMsg.substring(0, 200),
              });
              // Mark the lead with the error for DB tracking
              batch[j]._enrichmentError = errMsg.substring(0, 200);
            }
            processed++;
          }
          emitProgress('profiles', processed, leadsWithProfiles.length,
            `${batch[batch.length - 1].first_name} ${batch[batch.length - 1].last_name}`);
          i += concurrency;
        }
      }
  ...
  // --- Step 4.5: Find Missing Websites via Domain Guessing ---
  // Before crawling websites for emails, try to find websites for leads that don't have one
  {
    const leadsNeedingWebsite = leads.filter(l => !l.website && l.firm_name);
    if (leadsNeedingWebsite.length > 0 && leadsNeedingWebsite.length <= 200) {
      log.info(`Waterfall Step 4.5: Finding websites for ${leadsNeedingWebsite.length} leads`);
      emitProgress('website-find', 0, leadsNeedingWebsite.length, 'Finding firm websites');

      try {
        const { batchFindWebsites } = require('./website-finder');
        const wsStats = await batchFindWebsites(leadsNeedingWebsite, {
          onProgress: (current, total, name) => {
            emitProgress('website-find', current, total, name);
          },
          isCancelled,
          googleSearch: false, // DNS guessing only in waterfall (fast)
        });

        if (!stats.websitesFound) stats.websitesFound = 0;
        stats.websitesFound = wsStats.found;
        stats.totalFieldsFilled += wsStats.found;

        log.success(`Step 4.5 done: ${wsStats.found} websites found via domain guessing`);
      } catch (err) {
        log.warn(`Step 4.5 failed: ${err.message}`);
      }
    }
  }
  ...
  if (emailCrawl && emailFinder) {
    ...
    if (leadsNeedingEmail.length > 0) {
      ...
      try {
        ...
        log.error(`Step 5 email crawl failed: ${err.message}`);
      }
    }
  }
  ...
  // --- Step 6: SMTP Email Pattern Matching ---
  // For leads that STILL don't have an email but DO have a website,
  // try generating email patterns (first.last@domain, flast@domain, etc.)
  // and verifying via SMTP RCPT TO.
  // Runs independently of emailCrawl — zero dependency, just needs website domain.
  if (doSmtp) {
    const leadsStillNeedingEmail = leads.filter(l => !l.email && l.website);
    if (leadsStillNeedingEmail.length > 0) {
      log.info(`Waterfall Step 6: SMTP pattern matching for ${leadsStillNeedingEmail.length} leads`);
      emitProgress('smtp-patterns', 0, leadsStillNeedingEmail.length, 'Starting SMTP pattern matching');

      try {
        const EmailVerifier = require('./email-verifier');
        const verifier = new EmailVerifier();

        const smtpStats = await verifier.batchProcess(leadsStillNeedingEmail, {
          verifyExisting: false,
          findMissing: true,
          onProgress: (current, total, name) => {
            emitProgress('smtp-patterns', current, total, name);
          },
          isCancelled,
        });

        if (!stats.smtpPatternsFound) stats.smtpPatternsFound = 0;
        stats.smtpPatternsFound = smtpStats.found;
        stats.totalFieldsFilled += smtpStats.found;

        log.success(`Step 6 done: ${smtpStats.found} emails found via SMTP patterns`);
      } catch (err) {
        log.warn(`Step 6 failed: ${err.message} — continuing without SMTP patterns`);
      }
    }
  }
