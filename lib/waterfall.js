...
async function runWaterfall(leads, options = {}) {
  ...
  if (fetchProfiles) {
    const leadsWithProfiles = leads.filter(l => l.profile_url && needsEnrichment(l));
...
function findScraperForSource(source, SCRAPERS) {
  ...
  // Map source strings to scraper codes
  const sourceMap = {
    california_bar: 'CA',
    california: 'CA',
    oregon_bar: 'OR',
  ...
  for (const [key, code] of Object.entries(sourceMap)) {
