...
class SerperSearch {
   ...
   */
  /**
   * Geocode a location name to lat/lon via Nominatim (free, no API key)
   */
  async geocode(location) {
    return new Promise((resolve) => {
      const url = `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(location)}&format=json&limit=1`;
      https.get(url, { headers: { 'User-Agent': 'MortarMetrics/1.0' } }, res => {
        let body = '';
        res.on('data', c => body += c);
        res.on('end', () => {
          try {
            const data = JSON.parse(body);
            if (data[0]) resolve({ lat: parseFloat(data[0].lat), lon: parseFloat(data[0].lon) });
            else resolve(null);
          } catch { resolve(null); }
        });
  ...
  async findBusinesses(niche, location, maxResults = 100) {
    ...
    // Geocode location for better accuracy
    const coords = await this.geocode(location);
    const ll = coords ? `@${coords.lat},${coords.lon},13z` : '';

    // Serper Maps returns ~20 results per page. Use multiple queries for coverage.
    const queries = [
      `${niche} in ${location}`,
    ...
    for (const query of queries) {
      ...
      try {
        const results = await this.searchMaps(ll ? `${query}` : query);
