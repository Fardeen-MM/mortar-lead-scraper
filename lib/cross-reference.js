...
function mergeFields(lead, newData, sourceName) {
  ...
  for (const [key, value] of Object.entries(newData)) {
    if (!value) continue;
    if (!lead[key] || (typeof lead[key] === 'string' && lead[key].trim() === '')) {
      lead[key] = value;
      filled.push(key);
...
function similarity(a, b) {
  ...
  // Guard against division by zero (single-char strings have no bigrams)
...
function findNameMatch(lead, directoryResults, threshold = 0.8) {
  ...
  // Extract first name only (strip middle name/initial for better matching)
  const leadFirstFull = lead.first_name.toLowerCase().trim();
  const leadFirst = leadFirstFull.split(/\s+/)[0]; // "John Paul" → "John"
  ...
  for (const entry of directoryResults) {
    const entryFirstFull = (entry.first_name || '').toLowerCase().trim();
    const entryFirst = entryFirstFull.split(/\s+/)[0]; // strip middle names
    ...
    if (firstSim < 0.7) continue; // First name threshold (raised from 0.6 to reduce false matches)
