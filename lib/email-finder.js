 ...
 * Pattern: <a class="__cf_email__" data-cfemail="HEX">...</a>
 *   or:    <span class="__cf_email__" data-cfemail="HEX"></span>
 * Algorithm (public): first byte is XOR key, remaining bytes XOR'd with it.
 */
 ...
 * PATCHED 2026-04-16: Added Cloudflare email decode, HTML entity decode,
 * JS concatenation detection, reversed-string detection.
 */
async function extractEmailsFromPage(page) {
  ...
  try {
    ...
    // 5. Cloudflare email protection — decode data-cfemail attributes
    const cfHexStrings = await page.evaluate(() => {
      return Array.from(document.querySelectorAll('[data-cfemail], .__cf_email__'))
        .map(el => el.getAttribute('data-cfemail'))
        .filter(Boolean);
    });
    for (const hex of cfHexStrings) {
      const decoded = decodeCFEmail(hex);
      if (decoded && /[^@]+@[^@]+\.[a-z]{2,}/i.test(decoded)) {
        emails.add(decoded.toLowerCase());
      }
    }

    // 6. HTML entity decoded emails — &#64; for @, &#46; for .
    const fullHtml = await page.evaluate(() => document.documentElement.outerHTML || '');
    const entityDecoded = fullHtml
      .replace(/&#64;/gi, '@')
      .replace(/&#46;/gi, '.')
      .replace(/\[at\]/gi, '@')
      .replace(/\(at\)/gi, '@')
    ...
    entityEmails.forEach(e => emails.add(e.toLowerCase()));

    // 7. JS concatenation patterns — 'foo'+'@'+'bar.com' or string reverse
    // We look for specific fragments in the RAW HTML source
    const jsCatRe = /(['"])([a-zA-Z0-9._%+-]+)\1\s*\+\s*(['"])@\3\s*\+\s*(['"])([a-zA-Z0-9.-]+\.[a-zA-Z]{2,})\4/g;
    let m;
    while ((m = jsCatRe.exec(fullHtml)) !== null) {
      const e = (m[2] + '@' + m[5]).toLowerCase();
      if (e.length < 100) emails.add(e);
    }
