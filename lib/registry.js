...
const WORKING_SCRAPERS = new Set([
  ...
  'AVVO', 'AZ', 'CA', 'CA-AB', 'CA-BC', 'CA-CICC', 'CA-NL', 'CA-PE', 'CA-YT',
  'CO', 'CT', 'DE', 'ESA', 'FINDLAW', 'FL', 'FR', 'GA', 'GOOGLE-MAPS', 'GOOGLE-PLACES', 'HI', 'HK', 'ID', 'IE', 'IL', 'IT',
  'KY', 'LAWYERS-COM', 'MARTINDALE', 'MD', 'ME', 'MI', 'MN', 'MO', 'NC', 'NM', 'NZ', 'NY', 'OH', 'OR', 'PA',
  'SC', 'SG', 'TN', 'TX', 'UK-EW-BAR', 'UK-SC', 'VA', 'WA', 'WY',
  // Contractor scrapers (home services)
  'AZ-ROC', 'BUILDZOOM', 'CHECKATRADE', 'CSLB', 'CT-HIC', 'DBPR', 'FMB', 'GAS-SAFE', 'NICEIC', 'OR-CCB', 'TDLR', 'WA-LNI', 'NJ-HIC', 'NYC-HIC', 'PA-HIC', 'RBQ', 'TRUSTMARK', 'ATLANTIC-HW', 'HOUZZ', 'BBB', 'MANTA', 'YELP', 'WHICH-TRADERS',
  // Healthcare / Aesthetic
  'NPI', 'HEALTHGRADES', 'REALSELF', 'JCCP', 'HEALTHPOINT-NZ', 'NHS-UK',
  // Directories (Canada)
  'YP-CA',
  // Veterinary
  'RCVS-UK',
...
const SCRAPER_DIRS = [
  ...
  path.join(__dirname, '..', 'scrapers', 'contractors'),
];
    ...
    for (const file of files) {
      try {
        ...
        if (scraper && scraper.stateCode) {
          ...
          if (_registry[code]) {
            console.warn(`[registry] Duplicate stateCode '${code}': ${file} overwrites previous scraper`);
          }
